from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT_DIR = Path(__file__).resolve().parents[1]
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

from test_case_agent.new_tc_revision_decision_pack import (  # noqa: E402
    DEFAULT_PACKAGE_ID,
    build_new_tc_revision_decision_pack,
    write_new_tc_revision_decision_pack,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Build a read-only new TC revision decision pack.")
    parser.add_argument("--work-dir", required=True, type=Path, help="Directory containing pipeline artifacts.")
    parser.add_argument("--test-cases-dir", required=True, type=Path, help="Canonical test-cases directory, read-only.")
    parser.add_argument("--package-id", default=DEFAULT_PACKAGE_ID, help="Package id to analyze.")
    parser.add_argument("--old-version", default="autofin-prefinal", help="Old source version slug.")
    parser.add_argument("--new-version", default="autofin-final", help="New source version slug.")
    args = parser.parse_args()

    work_dir = args.work_dir
    pair = f"{args.old_version}-to-{args.new_version}"
    pack = build_new_tc_revision_decision_pack(
        package_id=args.package_id,
        revision_plan_path=work_dir / f"new-tc-draft-revision-plan-{args.package_id}.json",
        revision_plan_markdown_path=work_dir / f"new-tc-draft-revision-plan-{args.package_id}.md",
        draft_review_path=work_dir / f"new-tc-draft-review-{args.package_id}.json",
        draft_review_markdown_path=work_dir / f"new-tc-draft-review-{args.package_id}.md",
        draft_proposal_path=work_dir / f"new-tc-draft-proposal-{args.package_id}.json",
        draft_proposal_markdown_path=work_dir / f"new-tc-draft-proposal-{args.package_id}.md",
        context_bundle_path=work_dir / f"create-new-tc-context-bundle-{args.package_id}.json",
        context_bundle_markdown_path=work_dir / f"create-new-tc-context-bundle-{args.package_id}.md",
        requirements_diff_path=work_dir / f"requirements-diff.{pair}.json",
        impact_report_path=work_dir / f"impact-report.{pair}.json",
        update_plan_path=work_dir / f"test-case-update-plan.{pair}.json",
        old_registry_path=work_dir / f"requirements.{args.old_version}.jsonl",
        new_registry_path=work_dir / f"requirements.{args.new_version}.jsonl",
        test_cases_dir=args.test_cases_dir,
        created_by_tool="scripts/build_new_tc_revision_decision_pack.py",
    )
    json_path, markdown_path = write_new_tc_revision_decision_pack(pack, work_dir)
    readiness = pack.revised_draft_readiness
    payload = {
        "decision_pack_path": str(json_path),
        "decision_pack_markdown_path": str(markdown_path),
        "decision_pack_status": pack.decision_pack_status,
        "package_id": pack.package_id,
        "draft_decisions_count": len(pack.draft_decisions),
        "revise_ready_count": readiness.revise_ready_count,
        "replace_ready_count": readiness.replace_ready_count,
        "defer_count": readiness.defer_count,
        "maybe_extend_existing_tc_count": readiness.maybe_extend_existing_count,
        "needs_manual_decision_count": readiness.needs_manual_decision_count,
        "duplicate_risk_clusters_count": len(pack.duplicate_risk_clusters),
        "existing_tc_comparisons_count": len(pack.existing_tc_comparisons),
        "source_grounding_resolutions_count": len(pack.source_grounding_resolutions),
        "replacement_strategies_count": len(pack.replacement_strategies),
        "agent_capability_findings_count": len(pack.agent_capability_findings),
        "ready_for_revised_draft_proposal": readiness.ready_for_revised_draft_proposal,
        "canonical_write_allowed": pack.canonical_write_allowed,
        "manual_review_required": pack.manual_review_required,
        "warnings_count": len(pack.warnings),
        "blocking_reasons": pack.blocking_reasons,
    }
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
