...
MISSING_ENVIRONMENT_DATA_RE = re.compile(
    r"(?:fixture|фикстур)\w*\s+(?:отсутств(?:ует|уют|овал(?:а|и)?|овать)\w*|нет)|"
    r"(?:нет|отсутств(?:ует|уют|овал(?:а|и)?|овать)\w*)\s+(?:готов\w*\s+)?(?:партн[её]р\w*|реквизит\w*|сущност\w*|запис\w*|рол\w*|уч[её]тн\w*)|"
