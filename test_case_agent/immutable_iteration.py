    ...
    if stage == "writer":
        if request.get("writer_mode") == "model-runtime-prose":
            instruction = (
                ...
                "human wording only when needed. `preconditions` must contain "
                "only the exact sentinel `Не требуются.` or user/tester setup "
                ...
                "...`, `Поле доступно ...`, or `Блок отображается ...`. If a "
