...
class LeanV2BackendError(RuntimeError):
    pass
    ...
    def run_stage(
        ...
        *,
        ...
        try:
            ...
            raise LeanV2BackendError(f"{stage} output schema is invalid: {exc}") from exc
        ...
        if not resolution.verified:
            ...
            raise LeanV2BackendError(
                "no verified codex exec backend: "
                + (capability.error or ", ".join(capability.missing_flags))
            )
        started = time.perf_counter_ns()
        with tempfile.TemporaryDirectory(prefix=f"lean-v2-{stage}-") as raw_cwd:
            command = [
            ...
            try:
                ...
                raise LeanV2BackendError(f"{stage} process failed: {exc}") from exc
        ...
        if tool_events:
            raise LeanV2BackendError(f"{stage} emitted {tool_events} forbidden tool events")
        if not output_path.is_file() or not output_path.stat().st_size:
            raise LeanV2BackendError(f"{stage} completed without structured output")
        try:
            payload = json.loads(output_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            raise LeanV2BackendError(f"{stage} output is not JSON: {exc}") from exc
        if not isinstance(payload, dict):
            raise LeanV2BackendError(f"{stage} output must be a JSON object")
        try:
