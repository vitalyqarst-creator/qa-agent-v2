...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: compact scope brief с ранним `source-parity-check.md` при DOCX+PDF → русскоязычный `test-design-matrix.md` с классами покрытия → независимый matrix review в отдельной Codex-сессии → canonical TC в статусе review-ready только после `matrix-accepted` → независимый TC review в отдельной Codex-сессии → одна bounded revision → final baseline. Если пользователь просит “довести scope до тест-кейсов”, выполняй это как один macro-stage до accepted baseline или честного blocker-а, без остановок после внутренних matrix/review/writer handoff, когда следующий prompt уже материализован и не нужен внешний ответ. `test-design-matrix.md` — output writer-а под review, не source of truth; writer не создает и не обновляет `fts/**/test-cases/*.md` до принятого matrix review. XLSX-дубль не создается по умолчанию и нужен только по явному запросу. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом, не предлагаются как альтернатива и допускаются только по явному запросу.
