...
- Если основной ФТ доступен в DOCX и PDF, перед handoff к writer/reviewer по подтвержденному scope выполни `source-parity-check.md`; коды требований, найденные только в PDF, обязательны для `req_id` и трассировки. Канонический формат хранится в `references/agent/source-parity-check-format.md`.
