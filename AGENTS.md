...
- PDF используется только для structural/visual cross-check и не заменяет ни DOCX как source of truth, ни XHTML как mandatory machine-readable extraction source. Обычные support-файлы уточняют требования только внутри подтверждённого scope; package-level утверждённые решения БА, которые прямо изменяют требование ФТ, применяй по `references/agent/practical-v0.9-ba-decision-registry-format.md`; локальные макеты и доступные Figma-узлы помогают конкретизировать UI-шаги, но не задают бизнес-правила, обязательность, validation, allowed values или expected results.
...
- Каждый `coverage gap` должен явно указывать, к какому утверждению ФТ он относится: раздел, GSR/код требования при наличии, таблица/строка, поле/условие, цитата или atomic statement. Для practical v0.9 подробный формат gaps и ответов БА хранится в `references/agent/practical-v0.9-scope-obligations-format.md`; ответы пользователя по gaps фиксируй в `scope-clarification-requests.md`.
...
- Для обычной задачи «написать тест-кейсы по ФТ» используй compact practical route v0.9 из `references/agent/practical-test-case-route-v0.9.md` и skill `ft-practical-route`. Это один macro-stage до accepted baseline или честного external blocker. Канонические scope-artifacts: source-package manifest, source obligations, test-design matrix, scoped validator report, immutable review manifest/result, controller-owned review-session attestation и один `workflow-state.json`; не создавай self-check, WQG, stage summary, launch/dispatch receipts или дублирующие coverage-артефакты. Matrix review и final TC review всегда выполняются в отдельных верхнеуровневых Codex-сессиях над неизменяемыми snapshot. Для matrix и TC раздельно допускаются одна targeted writer revision и один fresh independent re-review; повторный содержательный verdict той же фазы блокирует scope. При source contradiction зафиксируй blocker и не выдумывай TC; отсутствие UI/data/observability отражай статусами `candidate-ui-calibration`, `needs-test-data`, `blocked-observability` или `needs-future-clarification`. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом.
...
- Перед independent review v0.9 создай immutable review manifest с branch/commit, hashes входов, нормализованными `repo_root`/`ft_package_root` и относительным путём FT-пакета; после создания отдельной reviewer-сессии controller записывает её фактический ID и те же roots в hash-bound attestation. Не проверяй код в одной папке и не пиши artifacts в другой молча. Для nested FT package все artifacts остаются внутри фактического `ft_package_root`. Перед matrix/TC обязательны DOCX и XHTML; доступный PDF связывай в source manifest и используй для structural/visual cross-check. Существующий `AGENT-NOTES.md` обязан быть hash-bound в source manifest. Scoped validator оценивает только dependency closure, явно связанную из `workflow-state.json`; `severity`, `blocking` и `remediation_owner` — разные поля. Process finding владельца `controller`/`validator` не расходует writer revision.
...
- Для межэтапного handoff v0.9 используй `workflow-state.json` как единственный mutable источник process-status; следующий агент читает его и canonical route, а не сохранённый prompt. New v0.9 scope folders находятся в `work/practical-v0.9/<scope-slug>/`. `workflow-state.yaml`, numbered stage-handoffs и `agent-decision-log.md` остаются контрактом только legacy route.
