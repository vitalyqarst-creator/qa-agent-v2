...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: scope brief → русскоязычный `test-design-matrix.md` → independent matrix review в отдельной Codex-сессии → stage summary → TC draft → independent TC review → при `tc-changes-required` одна bounded revision → final independent TC review в отдельной Codex-сессии → accepted baseline или честный blocker. Это один macro-stage без третьего TC review или автоматической дополнительной revision. До `matrix-accepted` или применимой round-cap policy canonical `fts/**/test-cases/*.md` не создавай/не обновляй. Если после bounded matrix repair остается `matrix-changes-required`, не блокируй scope автоматически: без source contradiction пиши TC с `candidate-ui-calibration` / `needs-test-data` / `blocked-observability` / `needs-future-clarification`; при source contradiction фиксируй blocker и не выдумывай TC. Дополнительный matrix review сверх разрешенного repair re-review, третий TC review, final-format review, semantic regression, XLSX-дубль, `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration или incremental update не являются default-маршрутом и допускаются только по явному запросу.
- Перед практическим writing/review фиксируй root consistency: `code_root`, `ft_package_root`, `artifact_write_root`, branch/commit. Detached HEAD допустим только когда exact commit совпадает с expected HEAD, а нужная ветка занята другим worktree; это нужно явно записать в summary. Если FT package/artifact root находится вне version-gated worktree и такой split явно не разрешен пользователем/контроллером, остановись как `blocked-input`; не проверяй код в одной папке и не пиши artifacts в другой молча. Для nested FT package вида `fts/<domain>/<ft-package>` все handoff/workflow/summary/test-case artifacts должны оставаться внутри фактического `ft_package_root`; не создавай `fts/<domain>/work/stage-handoffs` как workaround. Перед writer/matrix проверь физическое наличие `AGENT-NOTES.md`, DOCX, XHTML и PDF в `ft_package_root`. `practical-stage-summary.md` должен быть связан из `workflow-state.yaml` / package state внутри фактического package root и должен классифицировать validator errors как `next-stage-blocker`, `pre-existing-unrelated`, `validator-false-positive` или `mixed`, а validator warnings как `blocking-for-scope`, `expected-pre-writer`, `nonblocking-info` или `mixed`; при `errors_count > 0` нельзя безусловно писать `writer allowed`. При package-level `writer conditional` summary обязан содержать per-scope transitions, чтобы локальный warning одного scope не блокировал остальные.
