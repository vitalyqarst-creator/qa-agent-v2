...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: compact scope brief с ранним `source-parity-check.md` при DOCX+PDF → русскоязычный `test-design-matrix.md` с классами покрытия → один независимый matrix review в отдельной Codex-сессии → stage summary по accepted/blocked/round-cap scope → canonical TC в статусе review-ready только после `matrix-accepted` либо после practical round-cap policy с явными статусами → один независимый TC review в отдельной Codex-сессии → одна bounded revision → final baseline с явными статусами. Если matrix review возвращает `matrix-changes-required`, допускается один bounded matrix repair и один повторный matrix review; если после этого матрица не принята, не блокируй весь scope автоматически: если нет source contradiction, пиши TC с `candidate-ui-calibration` / `needs-test-data` / `blocked-observability` / `needs-future-clarification`; если есть source contradiction, фиксируй blocker и не выдумывай TC. Если пользователь просит “довести scope до тест-кейсов”, выполняй это как один macro-stage до accepted baseline, baseline after bounded revision или честного blocker-а, без остановок после внутренних matrix/review/writer handoff, когда следующий prompt уже материализован и не нужен внешний ответ. Независимый reviewer по умолчанию — отдельная Codex-сессия/task; sub-agent внутри того же turn не считается independent sign-off. Не запускай дополнительные matrix review, второй TC review, final-format review или semantic regression по умолчанию; они разрешены только при validator contract failure или явном запросе пользователя. `test-design-matrix.md` — output writer-а под review, не source of truth; writer не создает и не обновляет `fts/**/test-cases/*.md` до `matrix-accepted` или применимой practical round-cap policy. XLSX-дубль не создается по умолчанию и нужен только по явному запросу. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом, не предлагаются как альтернатива и допускаются только по явному запросу.
- Перед практическим writing/review фиксируй root consistency: `code_root`, `ft_package_root`, `artifact_write_root`, branch/commit. Если FT package/artifact root находится вне version-gated worktree и такой split явно не разрешен пользователем/контроллером, остановись как `blocked-input`; не проверяй код в одной папке и не пиши artifacts в другой молча. `practical-stage-summary.md` должен быть связан из `workflow-state.yaml` / package state и должен классифицировать validator errors как `next-stage-blocker`, `pre-existing-unrelated`, `validator-false-positive` или `mixed`; при `errors_count > 0` нельзя безусловно писать `writer allowed`.
