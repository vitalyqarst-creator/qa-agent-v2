...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: compact scope brief с ранним `source-parity-check.md` при DOCX+PDF → русскоязычный `test-design-matrix.md` с классами покрытия → один независимый matrix review в отдельной Codex-сессии → canonical TC в статусе review-ready только после `matrix-accepted` → один независимый TC review в отдельной Codex-сессии → одна bounded revision → final baseline с явными статусами. Если matrix review возвращает `matrix-changes-required`, допускается один bounded matrix repair и один повторный matrix review; если после этого матрица не принята, canonical TC не пишутся. Если пользователь просит “довести scope до тест-кейсов”, выполняй это как один macro-stage до accepted baseline, baseline after bounded revision или честного blocker-а, без остановок после внутренних matrix/review/writer handoff, когда следующий prompt уже материализован и не нужен внешний ответ. Не запускай дополнительные matrix review, второй TC review, final-format review или semantic regression по умолчанию; они разрешены только при validator contract failure или явном запросе пользователя. `test-design-matrix.md` — output writer-а под review, не source of truth; writer не создает и не обновляет `fts/**/test-cases/*.md` до `matrix-accepted`. XLSX-дубль не создается по умолчанию и нужен только по явному запросу. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом, не предлагаются как альтернатива и допускаются только по явному запросу.
