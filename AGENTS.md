...
- Для обычной задачи «написать тест-кейсы по ФТ» используй compact practical route v0.9 из `references/agent/practical-test-case-route-v0.9.md` и skill `ft-practical-route`. Это один macro-stage до accepted baseline или честного external blocker. Канонические scope-artifacts: source-package manifest, source obligations, test-design matrix, scoped validator report, immutable review manifest/result и один `workflow-state.json`; не создавай self-check, WQG, stage summary, launch/dispatch receipts или дублирующие coverage-артефакты. Matrix review запускай только по детерминированному complexity rule; final TC review всегда выполняется в отдельной верхнеуровневой Codex-сессии над неизменяемым snapshot. Один targeted writer revision и один fresh final independent TC review — максимум. При source contradiction зафиксируй blocker и не выдумывай TC; отсутствие UI/data/observability отражай статусами `candidate-ui-calibration`, `needs-test-data`, `blocked-observability` или `needs-future-clarification`. practical route v0.8 допускается только для продолжения существующего legacy run или по явному запросу. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом.
...
- `ft-practical-route` - default macro-route v0.9: исходники → обязательства → матрица → conditional matrix review → TC → final independent review.
