...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: compact scope brief → русскоязычный `test-design-matrix.md` с классами покрытия → canonical TC → review в отдельной Codex-сессии → одна bounded revision → final baseline. `test-design-matrix.md` — output writer-а под review, не source of truth. `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration и incremental update не являются default-маршрутом, не предлагаются как альтернатива и допускаются только по явному запросу.
