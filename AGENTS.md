...
- Для обычной задачи “написать тест-кейсы по ФТ” используй practical route v0.8 из `references/agent/practical-test-case-route-v0.8.md`: scope brief → русскоязычный `test-design-matrix.md` → independent matrix review в отдельной Codex-сессии → stage summary → TC draft → independent TC review → одна bounded revision → final baseline. Это один macro-stage до accepted baseline, baseline after bounded revision или честного blocker-а. До `matrix-accepted` или применимой round-cap policy canonical `fts/**/test-cases/*.md` не создавай/не обновляй. Если после bounded matrix repair остается `matrix-changes-required`, не блокируй scope автоматически: без source contradiction пиши TC с `candidate-ui-calibration` / `needs-test-data` / `blocked-observability` / `needs-future-clarification`; при source contradiction фиксируй blocker и не выдумывай TC. повторный matrix review, второй TC review, final-format review, semantic regression, XLSX-дубль, `benchmark`, `sharding`, `semantic bridge`, `source_assertion_review`, immutable `ft-agent run`, source-qualified iteration или incremental update не являются default-маршрутом и допускаются только по явному запросу.
