...
Для scope-local ответа, который уточняет ФТ и не отменяет его норму, после
получения ответа агент не помечает gap закрытым одним редактированием этого
файла. Он должен:
...
Если утверждённое решение БА или владельца продукта прямо отменяет либо меняет
норму ФТ, оно оформляется не карточкой `CLR-*`, а package-level реестром по
`practical-v0.9-ba-decision-registry-format.md` и имеет операционный приоритет
в явно указанной области действия. Если противоречие есть, но такого решения
нет, gap остаётся `open` типа `ba-decision-required` и в этом файле обязательно
создаётся вопрос к БА.
...
- БА заполняет в YAML-карточке поле «Ответ БА» (`user_response`).
