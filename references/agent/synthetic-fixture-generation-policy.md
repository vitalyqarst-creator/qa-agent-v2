...
свободный текст, дата, уникальное наименование и случайный нейтральный
идентификатор.

Это не обязательный источник данных и не часть runtime test execution. Для
закрытого справочника используй `DICT-*`; для интеграционного сценария DaData —
проверенный `FX-DADATA-*`; объекты, роли и статусы в стенде подготавливаются
отдельно.
