...
```json
{
  ...
  "routes": [
    ...
    },
    {
      ...
      "task_type": "Default macro-stage route for ordinary FT test-case writing: compact scope brief, Russian user-facing test-design matrix with explicit coverage classes from the coverage class catalog, one separate-session matrix review, canonical test cases after matrix-accepted or one bounded matrix repair, one separate-session TC review and one bounded revision, continuing across internal handoffs until accepted baseline, baseline after bounded revision, or an honest external blocker.",
      ...
      "verification_gates": ["scope-brief.md exists", "test-design-matrix.md exists with Russian headers and source-triggered classes from coverage-class-catalog.md", "no current canonical test-case file is created or updated before matrix-accepted or bounded matrix repair", "test-design-matrix-review.md exists from one separate reviewer session with matrix-accepted or matrix-changes-required", "matrix-changes-required has at most one matrix-repair-summary.md and no second matrix review by default", "canonical test-case file exists only after accepted matrix review or bounded matrix repair", "TC reviewer re-derived coverage from FT/PDF/XHTML/support instead of trusting the matrix", "review-findings.md exists or reviewer accepted without findings", "review-independence.md exists and proves separate reviewer sessions for matrix and TC review before independent sign-off", "macro-stage continues across internal matrix/review/writer handoffs without user confirmation when no external input is required", "no second TC review/final-format review/semantic regression unless validator contract failure or explicit user request", "no benchmark/sharding/bridge/immutable attempt artifacts are created"]
    },
    {
