...
- `static-closed` — ФТ/support задаёт полный конечный перечень; применяются правила
  полного покрытия значений.
- `external-dynamic` — авторитетный внешний контракт задаёт открытый изменяемый
  набор. Полный inventory не заявляется: для тест-дизайна обязателен небольшой
  curated fixture set из `fixture-catalog.md` со статусом проверки, снимком ответа,
  SHA-256 и правилом устаревания.

Для `external-dynamic` в `notes` фиксируй provider, официальный контракт,
параметры/границы запроса, `coverage_mode: reference-only` и ссылки на все
`FX-*`. Значения нельзя получать во время выполнения TC или придумывать writer-ом.
