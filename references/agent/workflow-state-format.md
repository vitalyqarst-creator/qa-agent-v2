...
- Если source-selection зарегистрировал Figma visual reference, `latest_artifacts.figma_visual_discovery` должен указывать на `figma-visual-discovery.md`. Его `discovery_status` (`available | partial | unavailable`) фиксируется в artifact; `unavailable` не является blocker и не требует Figma как `required_input` writer-а.
