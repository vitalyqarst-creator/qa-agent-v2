...
`practical-v0.9` — legacy-маршрут для уже начатых v0.9 scope. Для новых
обычных scope используй `practical-v1`; этот документ не является fallback.

Утверждённые ответы БА для legacy scope оформляются по
[формату реестра решений БА](practical-v0.9-ba-decision-registry-format.md).
...
границ или состояний. `Исходное состояние` не содержит trigger;
`Формирование состояния` задаёт его один раз либо указывает предусловие;
`Проверяемое действие` вызывает результат. Matrix gate проверяет primary
oracle create/edit, file-итерации и source-defined границы.
В «Нужных предпосылках» перечисляй все связанные бизнес-предпосылки.
...
свойстве, действии или primary oracle. Семантические дубли разных OBL-*
требуют CON-* с одним из этих решений.

До validator: source-to-matrix scan из `ft-test-case-writer`, без артефакта.
