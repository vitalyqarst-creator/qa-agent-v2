...
2. `../source-package-manifest.json` — DOCX, XHTML, PDF, package notes и их SHA-256;
...
- DOCX — источник смысла; XHTML — обязательное машиночитаемое представление; PDF — обязательная structural/visual cross-check. Отсутствие любого из трёх блокирует downstream stage.
...
```text
python scripts/validate_practical_scope.py --ft-package-root <package> --scope-manifest <scope-dir>/workflow-state.json --output-profile <scope-dir>/validator-report.json --exclude-output <scope-dir>/validator-report.json --require-clean
```

Matrix review обязателен только если в `scope-obligations.json` не менее 8 обязательств или есть `risk_flags`: `status-transition`, `cross-field-rule`, `closed-dictionary`, `integration`, `authorization`, `high-risk`. Для простого scope matrix review не запускается: после чистой валидации writer переходит к TC.
...
```text
python scripts/create_practical_review_manifest.py --repo-root <repo> --ft-package-root <package> --scope-manifest <scope-dir>/workflow-state.json --review-mode <matrix|test-cases> --controller-thread-id <current-top-level-thread-id> --contract-file references/agent/practical-test-case-route-v0.9.md --output <scope-dir>/<mode>-review-manifest.json
```

Reviewer запускается в новой верхнеуровневой Codex-сессии (`codex-thread`), read-only для matrix/TC. До чтения matrix и TC он самостоятельно формирует в `<mode>-review-result.json` массив `independent_obligations`, сверяя его с DOCX/XHTML/PDF. Не передавай writer self-check: такого артефакта в v0.9 нет.
...
`source-integrity`, `unresolved-requirement`, `semantic-completeness`, `traceability`, `execution-readiness`, `review-integrity` и `artifact-tampering` блокируют. `format`, `style`, `transport` и `advisory-risk` не блокируют сами по себе; они остаются видимыми и могут быть подняты до blocker только явным правилом.

## Compatibility

Workflow хранит `route_version` и manifests хранят contract version + SHA-256 входов. Git commit фиксируется в review manifest для аудита, но изменение нерелевантного agent-layer кода не инвалидирует scope, если contract version и input hashes не изменились.
