...
- Разные `OBL-*` не оправдывают отдельные TC сами по себе. Одинаковые
  контекст, контрол, состояние, действие и oracle — candidate semantic
  duplicate: validator предупреждает, reviewer подтверждает различие либо
  назначает один planned TC-ID. Тогда TC трассирует все связанные `OBL-*`/
  `SCN-*` и имеет один oracle. Не склеивай разные create/edit-потоки,
  состояния, классы значений или результаты.
...
подсказки или одного автозаполненного поля.
