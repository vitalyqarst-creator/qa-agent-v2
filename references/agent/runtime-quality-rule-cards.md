...
| R-ATOMICITY | TC references more than two independent source-backed obligations (`ATOM`/`BSR`/`GSR`/`REQ`) | split, or add explicit scenario rationale only when one user workflow is source-backed and atomic coverage remains traceable elsewhere | `test-case-overmerged-atoms-without-rationale`, `test-case-excessive-atom-fan-in` |
