...
| R-REP-DATA | representative strategy states selected classes | strategy text must match actual concrete values and invalid classes | `representative-strategy-data-mismatch`, `sampled-field-group-without-group-strategy` |
| R-NEG-TRIGGER | candidate-negative TC enters invalid value | include neutral completion/focus/validation/save trigger or explicit trigger BA question | `candidate-negative-validation-trigger-missing` |
| R-PERSISTENCE-COVERAGE | editable CRUD/card scope | add save/persist smoke coverage plan or explicit out-of-scope rationale | `persist-coverage-missing-for-crud-scope` |
