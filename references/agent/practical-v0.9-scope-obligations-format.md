...
Если кроме первичного статуса есть недоступная предпосылка с другим статусом,
она не пропадает: добавь в matrix и связанный TC поле «Ограничения
исполнения» с русским пояснением и всеми соответствующими `SETUP-*`. Это не
второй «Статус исполнения» и не разрешает называть неподтверждённый
UI-контрол установленным фактом.

Если `SETUP-*` имеет `kind: fixture` и `availability: provided`, обязательное
поле `artifacts` перечисляет каждый файл, подтверждающий использованные
literals: например response snapshot, verification receipt и при необходимости
fixture catalog. Пути задаются относительно корня FT-пакета. Эти файлы входят
в immutable manifest независимого review; текстовой ссылки на `FX-*` в поле
...
Если ФТ задаёт один точный общий результат отказа для нескольких независимых
классов невалидного ввода, обязательство с сообщением содержит
`common_result_for_obligation_ids` с полным списком связанных `OBL-*`.
Например, один текст ошибки применяется к неверному формату и превышению
размера файла. Каждая строка matrix и каждый TC этих классов воспроизводят
точный общий результат; нельзя проверять сообщение лишь на одном примере.
