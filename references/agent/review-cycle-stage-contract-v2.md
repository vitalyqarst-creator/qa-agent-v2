...
The optimized prepared writer and reviewer are allowlisted only for package version 8 with `execution_profile = simple-field-property`, no unsupported dimensions and no blocking coverage gaps. Versions 1 through 7 remain readable as immutable legacy evidence but are not runnable as the current prepared contract. Package version 8 retains the v7 machine-readable `OBL-* -> ATOM-* -> TC/GAP`, exhaustive dictionary, calibration and input-fingerprint contracts and adds first-class exact `reference-only` fixture values with hierarchy paths. All table-parity, numeric-boundary, integration/persistence and dependency/state packages route to the standard writer/reviewer contracts. Prepared execution uses the compact technical projections in `prepared-writer-runtime-profile.md` and `prepared-reviewer-runtime-profile.md`; the full skills and deep reviewer rubric remain authoritative outside this narrow allowlist.
