...
The optimized prepared writer and reviewer are allowlisted only for package version 4 with `execution_profile = simple-field-property`, no unsupported dimensions and no blocking coverage gaps. Versions 1, 2 and 3 remain readable as immutable legacy evidence but route out of the fast path. Package version 4 preserves explicit `OBL-* -> ATOM-* -> TC/GAP` relations and non-blocking constraint gaps. All table-parity, numeric-boundary, integration/persistence and dependency/state packages route to the standard writer/reviewer contracts. Prepared execution uses the compact technical projections in `prepared-writer-runtime-profile.md` and `prepared-reviewer-runtime-profile.md`; the full skills and deep reviewer rubric remain authoritative outside this narrow allowlist.
