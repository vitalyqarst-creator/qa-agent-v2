...
До выбора одного scope карта должна показать, куда попадёт каждое явное
...
- каждый формальный код требования (`AS.*`, `BSR *`, `GSR *`) указывается
  ровно в одной строке распределения: в candidate scope, `cross-scope` или
  `out-of-scope`;
...
- общие правила раздела, стоящие до подразделов, нельзя молча потерять между
  candidate scope. Их нужно назначить существующему scope или выделить в
...
- Карта candidate scope не готова, пока в `Распределение требований ФТ` не назначены все формальные коды из выбранных основных источников. Не заменяй это фразой «остальное относится к разделу».
