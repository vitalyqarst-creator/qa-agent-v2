...
- Ожидаемый результат negative/validation TC допускает несколько реакций через `или`, например `символ отклонен или значение остается незаполненным/предыдущим`, вместо одного подтвержденного deterministic observable oracle или `GAP-*`.
...
- Шаг содержит несколько возможных UI-лейблов или действий через `/`, `alias`, `или`, например `Добавить дополнительный доход / Добавить источник дохода`, вместо одного наблюдаемого label/action.
...
- Numeric-only, length, mask или другой input restriction TC не выбирает один изолированный oracle: подтвержденный `field-state`, `transition-state` с полным valid fixture, source-backed marker/message или узкий `GAP-*`.
...
- Исполнимый `TC-*` использует конкретное source-bound доменное значение без доказанного provenance. Если значение влияет на интеграцию, справочник, поиск/фильтрацию, автозаполнение, статус/права, backend mapping или другой бизнес-результат, reviewer должен требовать verified fixture / source literal / dictionary value / approved evidence либо переводить кейс в `needs-test-data`, `candidate-ui-calibration` или `GAP-*`.
- Исполнимый `TC-*` использует fixture/source поле как expected value другого семантического UI-поля без source/support/UI evidence для такого mapping.
...
7x. `nondeterministic-alternative-oracle`: negative/validation expected result допускает альтернативные реакции через `или` вместо одного deterministic oracle или `GAP-*`.
7y. `executable-over-unresolved-mechanism`: исполнимый TC ссылается на unresolved `GAP-*` по ключевому action/feedback/oracle, но все равно заявляет pass/fail baseline.
7z. `ambiguous-ui-alias-step`: шаг использует несколько UI labels/actions через `/`, `alias` или `или` вместо одного наблюдаемого действия.
7aa. `derived-setup-behavior-as-source`: setup/recovery behavior без source evidence представлен как source-backed baseline coverage.
7ab. `non-reproducible-precondition`: magic/vague/passive `Предусловия` без action setup, fixture/API/profile; `Убедиться...` / `Дождаться...` alone is not setup.
7ac. `production-runtime-not-self-contained`: production TC has setup profile references, stand/environment or package-name leakage, missing reveal action for action-created fields, or embedded diagnostic/design sections.
7ad. `missing-equivalence-class`: source-backed input restriction has no separate source-derived class coverage for whitespace, alphabet/script class, special/disallowed symbols or boundaries.
...
20b.1. TC с конкретными доменными test data доказывает provenance каждого source-bound значения. Neutral synthetic value допустим только для unrestricted free-text; если поле или объект source-bound, reviewer требует fixture/source/dictionary/evidence или статус `needs-test-data` / `candidate-ui-calibration`.
20b.2. Для positive create/save reviewer отдельно проверяет lifecycle данных в системе: конкретный ключ объекта, исходное `absent`/`existing`, cleanup или изоляцию прогона. Provider fixture, включая `FX-DADATA-*`, не доказывает отсутствие системного объекта; повтор ключа между успешными созданиями без cleanup — blocking finding.
20b.3. Для отмены/закрытия без сохранения reviewer проверяет source effect целиком: кейс должен иметь follow-up observation и явно утверждать, что объект не создан или изменение не сохранено. Открытие новой пустой формы само по себе не доказывает отсутствие persistence.
