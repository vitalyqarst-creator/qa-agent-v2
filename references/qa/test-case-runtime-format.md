...
- Повторное использование одного проверенного набора данных в однотипных
  кейсах допустимо и не требует искусственно разных значений. В «Тестовые
  данные» включай только literals, которые нужны для шагов, итогового
  ожидаемого результата либо заполнения обязательных полей перед сохранением
  или переходом. Не копируй полный профиль интеграционной организации в
  кейс, который проверяет только один запрос, подсказку или автозаполненное
  поле; и не заменяй конкретные данные одной ссылкой на fixture.
- Не используй process placeholders вроде `Несохранённое значение` или
  `edit TC`; укажи конкретный literal с подтверждённым происхождением.
