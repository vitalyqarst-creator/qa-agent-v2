...
Все человекочитаемые поля findings artifact и writer response artifact должны
быть заполнены на русском языке. В новых artifacts используй русские видимые
названия полей из шаблонов ниже; English labels поддерживаются валидатором
только для совместимости со старыми artifacts. Допустимые enum-значения и
machine keys остаются в каноническом виде.
...
- `coverage_dimension` = `role-permission | status-lifecycle | decision-table | pairwise | boundary | equivalence | dependency | conditional-visibility | api-server-validation | integration | security | async | persistence | cross-view-projection | audit-history | table-list | file-upload | calculation | numeric | date-time | length | scenario-use-case | performance | reliability | compatibility | usability | accessibility-ui | traceability | expected-result | atomarity | format | scope | other`
- `status` = `open`
...
### FINDING-001
**Режим ревью:** structure
**Критичность:** error
**Категория:** atomarity
**Измерение покрытия:** atomarity
**Идентификатор тест-кейса:** TC-DEMO-001
**Ссылка трассировки:** -
**Заголовок:** Невалидная комбинация двух проверок в одном тест-кейсе

**Проблема:** Один тест-кейс одновременно проверяет обязательность поля и ограничение по допустимым символам.

**Доказательства:**
- В кейсе объединены две независимые проверки.
- Нарушено правило `Один тест-кейс = одна проверка`.

**Требуемое изменение:** Разделить комбинированный тест-кейс на два отдельных атомарных кейса.

**Ссылка на источник:** `ФТ 2`; `2.1.1.1.1.1.2`; `Поле «ФИО клиента»`
...
**Сводка изменений:** Комбинированный кейс разделен на два отдельных кейса по обязательности и по допустимым символам.

**Затронутые ID тест-кейсов:**
- `TC-DEMO-001`
- `TC-DEMO-002`

**Затронутые ссылки трассировки:**
- `-`
```
