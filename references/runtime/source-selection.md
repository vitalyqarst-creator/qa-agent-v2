...
PDF/рендер при наличии проверяется визуально и структурно, но не становится источником бизнес-правил. Рендеры и другие временные файлы создаются только в отдельном системном временном каталоге `ft-runtime-<назначение>-<случайный-id>`. Для безопасной неинтерактивной очистки source locator вызывает `python scripts/cleanup_runtime_temp.py <absolute-temp-path>`; ручные рекурсивные команды удаления не используются. Новые файлы в repo-local `tmp/` после регистрации source-locator считаются ошибкой этапа.
