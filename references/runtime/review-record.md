...
Каждый finding нового matrix review версии 3 содержит `origin_stage`: `scope`, если дефект уже присутствует в analyzer-owned inventory/brief/gap/data baseline; `matrix`, если analyzer handoff достаточен, а дефект внесён writer-ом при проектировании matrix/data plan; `both`, если исправить нужно оба слоя. Validator возвращает `repair_stage: scope`, если встречается `scope|both`, иначе `repair_stage: matrix`. Controller маршрутизирует исправление только по этому полю и не интерпретирует текст findings.
