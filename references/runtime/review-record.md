...
Matrix и canonical TC проверяются в отдельных верхнеуровневых Codex-сессиях. Reviewer не создаёт собственную сессию и не формирует dispatch evidence: запуском и регистрацией владеет controller.
...
3. Полученный от `create_thread` `threadId` controller регистрирует командой `scripts/runtime_review_dispatch.py create`. Receipt имеет имя `<kind>-review-dispatch-<artifact-hash-prefix>.json`.
...
Controller не использует subagent, fork или текущую writer-сессию как независимый reviewer. Если thread API недоступен или пользователь ещё не разрешил создание отдельных сессий, route останавливается до получения возможности/разрешения. Разрешение запрашивается один раз на текущий practical route.
