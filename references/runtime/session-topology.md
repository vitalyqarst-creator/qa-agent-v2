...
Для scope-validator controller использует `writer_allowed`, `blocking_errors`, `quality_findings`, `scope_revision_count`, `correction_allowed`, `final_closure_allowed` и `workflow_status` из JSON. При blockers и count `0` или `1` controller возвращает analyzer-у полный текущий JSON для ограниченной содержательной коррекции; analyzer увеличивает count. При count `2` дополнительный проход допустим только когда validator явно вернул `final_closure_allowed=true`: не более двух локализованных blocker-ов атомарности, полноты, трассировки или source-contract. Analyzer меняет count на `3`; оставшаяся ошибка завершает этап. При `writer_allowed=true` controller переходит к matrix, даже если есть `quality_findings`: они проверяются matrix writer-ом и обязательным independent matrix review. Controller не называет смысловые findings форматными и не запускает по ним новый scope-цикл.
