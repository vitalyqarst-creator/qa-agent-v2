...
Если matrix re-review возвращает `review_quality_status: failed-prior-review-incomplete`, controller останавливает текущий route как `review-quality-failed`: новые findings сохраняются, но не расходуют ещё один обычный revision cycle. Controller не исправляет их сам и не продолжает к TC; это сигнал улучшить reviewer/validator и выполнить новый независимый прогон, а не многократно раскрывать findings по одному.

Для scope-validator controller использует `writer_allowed`, `blocking_errors`, `quality_findings`, `scope_revision_count`, `correction_allowed` и `workflow_status` из JSON. При blockers и count `0` controller возвращает analyzer-у JSON для одной локальной структурной коррекции; analyzer меняет count на `1`. Оставшийся blocker при count `1` завершает этап как `scope-stage-failed`; второй scope-correction нет. При `writer_allowed=true` controller переходит к matrix, даже если есть `quality_findings`: они проверяются matrix writer-ом и обязательным independent matrix review. Controller не называет смысловые findings форматными и не запускает по ним новый scope-цикл.
