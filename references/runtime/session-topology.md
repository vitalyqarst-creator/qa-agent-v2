...
Для scope-validator controller использует `writer_allowed`, `blocking_errors`, `quality_findings`, `scope_revision_count`, `correction_allowed` и `workflow_status` из JSON. При blockers и count `0` или `1` controller возвращает analyzer-у полный текущий JSON для ограниченной содержательной коррекции; analyzer увеличивает count на единицу. Оставшийся blocker при count `2` завершает этап как `scope-stage-failed`; третьей scope-correction нет. При `writer_allowed=true` controller переходит к matrix, даже если есть `quality_findings`: они проверяются matrix writer-ом и обязательным independent matrix review. Controller не называет смысловые findings форматными и не запускает по ним новый scope-цикл.
