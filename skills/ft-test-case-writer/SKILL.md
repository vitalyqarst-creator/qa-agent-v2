...
8f.1. Для нейтральных значений свободного ввода/формата можно по явной необходимости создать неизменяемый `FX-SYNTH-*` через [`synthetic-fixture-generation-policy.md`](../../references/agent/synthetic-fixture-generation-policy.md). Это одноразовый snapshot с конкретными literals, а не live dependency: TC и автотест не вызывают API. Не используй его для DaData, существования организации/адреса, бизнес-валидности реквизитов, ролей, статусов или интеграционных expected results.
