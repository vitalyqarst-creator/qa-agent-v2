...
8. Перед передачей matrix на independent review выполни один ограниченный
   ...
   докажи успешное сохранение повторным наблюдением объекта. Для source-defined
   формата и границ входа перечисли независимые negative-классы, а не один
   пример. Для файлов укажи точное начальное состояние контейнера и изоляцию
   каждой параметрической итерации. Не помещай уже внесённое изменение в
   «Исходное состояние»: оно формируется ровно один раз перед действием. В
   create/edit-контексте primary oracle относится только к своему потоку, без
   альтернативы «создано или изменено». Наконец, классифицируй CON-* не только
   для дословных дублей, но и для сценариев с одинаковыми элементом, действием
   и наблюдаемым результатом из разных OBL-*. Не создавай для этого отдельный
   self-check-файл.
