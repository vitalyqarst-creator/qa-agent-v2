...
- If the user asks to write test cases for a normal FT scope, use practical route v0.8 as one macro-stage: `ft-source-locator` -> `ft-scope-analyzer` with early DOCX/PDF source parity when available -> `ft-test-case-writer` in matrix-only mode -> one separate-session `ft-test-case-reviewer` matrix review -> required matrix-stage summary -> `ft-test-case-writer` TC draft after `matrix-accepted` or after practical round-cap policy allows explicit-status TC -> one separate-session `ft-test-case-reviewer` TC review, then one bounded revision pass if blocking findings remain. If matrix review returns `matrix-changes-required`, allow one bounded matrix repair and one matrix re-review; if the matrix is still not accepted, block only `source_contradiction: yes` (source contradiction) / unrepresentable obligations and otherwise write TC with `candidate-ui-calibration`, `needs-test-data`, `blocked-observability` or `needs-future-clarification`. Continue across these internal gates until accepted baseline, baseline after bounded revision, or an honest external blocker; do not stop after each materialized handoff when no user/BA decision is required. Separate-session means a real Codex task/thread; sub-agents do not count for independent sign-off. The matrix-stage summary must record `code_root`, `ft_package_root`, `artifact_write_root`, split-root approval when applicable, validator error/warning classification, per-scope transitions for package-level conditional routing, source restore provenance/SHA-256 when package files were restored, `source_contradiction: yes/no` for round-cap scopes, and `next_stage_transition`; after TC writing, use `tc-review allowed` or `tc-review conditional`. Link the summary from affected workflow/package state inside the actual FT package root. Do not start extra matrix reviews, a second TC review, final-format review or semantic regression by default. Markdown `test-design-matrix.md` is enough by default; XLSX export is optional only by explicit request.
