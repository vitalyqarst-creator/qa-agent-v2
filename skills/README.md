...
- If the user asks to write test cases for a normal FT scope, use practical route v0.8 as one macro-stage. Chain: locator -> analyzer -> matrix writer -> one separate-session matrix review -> summary -> TC writer -> one separate-session TC review -> one bounded revision if needed; no second TC review by default. Matrix repair allows one matrix re-review. Continue until accepted baseline, baseline after bounded revision with explicit residual statuses, or an honest external blocker. Round-cap blocks only source contradiction / unrepresentable obligations; missing data/UI/observability becomes `candidate-ui-calibration`, `needs-test-data`, `blocked-observability` or `needs-future-clarification`. Separate-session means a real Codex task/thread; sub-agents do not count. Full gates live in `references/agent/practical-test-case-route-v0.8.md`.
