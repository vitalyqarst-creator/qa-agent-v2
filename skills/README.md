...
- If the user asks to write test cases for a normal FT scope, use `ft-practical-route` v1. Chain: source scope and BA questions -> matrix -> mandatory separate-session matrix review -> TC -> final separate-session TC review. Each review phase permits one complete writer revision and one short check that its findings are closed. Separate-session means a real top-level Codex thread; sub-agents do not count. Missing data/UI/observability receives `needs-test-data`, `candidate-ui-calibration` or `blocked-observability` and does not stop unambiguous tests. Full contract: `references/agent/practical-test-case-route-v1.md`.
