...
| `ft-scope-analyzer` | Новый раздел ФТ | Границы scope, source-level gaps, вопросы БА, план тестовых данных |
| `ft-test-case-writer` | Подтверждённый scope | Спецификация данных и внешние snapshots → matrix → canonical TC по явному переходу |
...
- `references/runtime/test-data-fixtures.md` — спецификация данных, готовность среды и сохранение внешних ответов;
...
Нормализация канонического DOCX выполняется `scripts/normalize_ft_source.py`. Обязательные проверки переходов: `scripts/runtime_session_registry.py`, `scripts/validate_runtime_source.py`, `scripts/validate_runtime_scope.py`, `scripts/validate_runtime_matrix.py` с inventory/gaps и `scripts/validate_runtime_tc.py` с matrix. Review/re-review используют `scripts/runtime_review_dispatch.py`, `scripts/runtime_review_delta.py` и `scripts/validate_runtime_review.py`.
