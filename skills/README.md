...
- If the user asks to write test cases for a normal FT scope, use practical route v0.8 as one macro-stage: `ft-source-locator` -> `ft-scope-analyzer` with early DOCX/PDF source parity when available -> `ft-test-case-writer` in matrix-only mode -> separate-session `ft-test-case-reviewer` matrix review -> `ft-test-case-writer` TC draft only after `matrix-accepted` -> separate-session `ft-test-case-reviewer` TC review, then one bounded revision pass if blocking findings remain. Continue across these internal gates until accepted baseline or an honest external blocker; do not stop after each materialized handoff when no user/BA decision is required. Markdown `test-design-matrix.md` is enough by default; XLSX export is optional only by explicit request.
...
- New test-case suite, macro-stage default: `ft-source-locator` -> `ft-scope-analyzer` -> `ft-test-case-writer` matrix-only -> separate-session `ft-test-case-reviewer` matrix review -> `ft-test-case-writer` TC draft -> separate-session `ft-test-case-reviewer` TC review using practical route v0.8, continuing without user prompts until accepted baseline, one bounded revision with explicit residual statuses, or an external blocker.
