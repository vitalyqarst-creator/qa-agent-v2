...
- If the user asks to write test cases for a normal FT scope, use the practical route v0.8: `ft-source-locator` -> `ft-scope-analyzer` with early DOCX/PDF source parity when available -> `ft-test-case-writer` in matrix-only mode -> separate-session `ft-test-case-reviewer` matrix review -> `ft-test-case-writer` TC draft only after `matrix-accepted` -> separate-session `ft-test-case-reviewer` TC review, then one bounded revision pass if blocking findings remain. Markdown `test-design-matrix.md` is enough by default; XLSX export is optional only by explicit request.
...
- New test-case suite, default: `ft-source-locator` -> `ft-scope-analyzer` -> `ft-test-case-writer` matrix-only -> separate-session `ft-test-case-reviewer` matrix review -> `ft-test-case-writer` TC draft -> separate-session `ft-test-case-reviewer` TC review using practical route v0.8.
