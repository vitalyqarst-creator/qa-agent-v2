...
- New test-case suite, macro-stage default: `ft-source-locator` -> `ft-scope-analyzer` -> `ft-test-case-writer` matrix-only -> one separate-session `ft-test-case-reviewer` matrix review -> matrix-stage summary -> `ft-test-case-writer` TC draft -> one separate-session `ft-test-case-reviewer` TC review using practical route v0.8, continuing without user prompts until accepted baseline, baseline after one bounded revision with explicit residual statuses, or an external blocker.
