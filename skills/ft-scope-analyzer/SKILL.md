...
- перечислить `scope-contract.md`, `scope-coverage-gaps.md`, `workflow-state.yaml`, conditional `source-parity-check.md`, `source-row-inventory.md`, `mockup-visual-inventory.md`, `AGENT-NOTES.md` и regression/baseline artifacts;
- не запускать writer, если обязательный parity/row/mockup artifact должен быть, но отсутствует или mockup inventory имеет `opened != yes`;
- указать writer mode: `continue-current-workflow`, `revision-from-findings`, `rebuild-from-scope` или `fresh-eval-run`;
- требовать package-by-package writer-pass: минимум один `WP-*` в `scope-contract.md`, `package_id` у `ATOM-*`/`TC-*`, gate order `package ledger self-check -> Package Test Design Plan self-check -> package TC self-check`;
- для table/PDF/DOCX extraction требовать `Source Table Normalization` до ledger; table residue, соседние поля и low-confidence rows идут в `GAP-*`;
- требовать `Artifact Write Strategy` and `scripts/write_artifact_sections.py --manifest <manifest.json>` for large/package-based artifacts; one-shot giant writes, compact drafts and requirement merging are blockers;
- запретить silently promote to `covered` previous `gap` / `unclear` or internal/API/async assertions without new observable artifact gate;
- перечислить outputs/gate: canonical TC, ledger, matrices, writer self-check, `prompt.writer-to-reviewer.round-1.md`, `stage_status: ready-for-review`; writer не ставит `signed-off`.
