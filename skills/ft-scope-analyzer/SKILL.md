...
не вопрос к БА. Каждый зависящий atom/row получает тот же статус и записывается
в таблицы `Предпосылки исполнения` и `Зависимости от тестовых данных` scope brief.
В первой для каждого atom укажи исполнителя, объект/исходное состояние и статус.
