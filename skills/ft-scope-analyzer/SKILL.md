...
- перечислить `negative-oracle-inventory.md` / `requiredness-oracle-inventory.md`, если scope содержит validation/format ограничения или обязательность; если нужный artifact отсутствует, не запускай writer и зафиксируй blocking input issue;
...
10b. Если oracle отсутствует, создай/link `GAP-*` и clarification request, присвой stable `scope_obligation_id` (`SO-NEG-*` / `SO-REQ-*`) и перенеси его в `scope-coverage-gaps.md` через `Scope Obligation ID(s)`. Если общий validation standard / UX/API rulebook не найден, укажи `oracle_source = not_found`; не оставляй это на writer.
