...
3. Сохраняй один `OBL-*` на независимое утверждение ФТ и один основной
   ...
   Повторно используй один `CTX-*` для нескольких OBL одного пользовательского
   потока, если trigger, взаимодействие и требуемые setup одинаковы: например,
   выбор одной DaData-подсказки и все поля, которые она заполняет. Не создавай
   отдельный CTX только потому, что изменяется другое поле результата. Create
   и edit остаются разными CTX, когда это действительно разные потоки.
...
8. Для поля с типом значения `Дата` создай независимые positive и negative
   обязательства для каждого применимого контекста. Negative-обязательство
   использует невозможную календарную дату; если механизм отклонения не задан
   источником, передай его writer-у как `candidate-ui-calibration`, а не как
   отказ от проверки.
