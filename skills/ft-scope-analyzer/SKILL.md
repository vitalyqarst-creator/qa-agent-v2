...
1. Восстанови структуру по DOCX/XHTML; PDF используй для structural/visual
   cross-check. Если в пакете есть Figma-запись, релевантная scope, проверь её
   до матрицы: используй `node-id`, если он указан, иначе сам найди
   страницу/фрейм по названию scope, экрана или элемента. Не требуй `node-id`
   от пользователя. Сделай не более двух попыток: открыть точную ссылку,
   дождаться кадра; затем при неудаче один раз перезагрузить ту же ссылку с
   увеличенным ожиданием. В `source-scope.md` зафиксируй страницу/фрейм, число
   попыток и итог `checked-and-used`, `checked-no-new-information`,
   `visual-check-failed` или `not-relevant`; `not_checked` после анализа
   недопустим. `visual-check-failed` не блокирует работу и не означает, что
   файл Figma недоступен: используй ФТ/PDF/локальный макет. Если Figma была
   единственным источником нужного UI-факта, создай `BAQ-*` либо пометь
   конкретную проверку `candidate-ui-calibration`. Макет/Figma может
   подтвердить UI-элемент, но не бизнес-правило.
