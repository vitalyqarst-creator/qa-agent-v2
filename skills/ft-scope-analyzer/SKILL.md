...
неразрешенном продуктовом правиле/поведении; тестовый account/object/fixture или
роль с уже определенными source правами — `needs-test-data` с точными свойствами,
не вопрос к БА. Каждый зависящий atom/row получает тот же статус в таблицах
`Предпосылки исполнения` и `Зависимости от тестовых данных`. В первой укажи
исполнителя, состояние, `SETUP-*`, статус и доказательство: `FX-*`, шаги ФТ с
данными или отсутствующая предпосылка. ATOM одной команды разделяют setup;
отсутствующий setup — `needs-test-data`. `Тестировщик` / «создать или выбрать»
не доказывают прав или подготовки.
