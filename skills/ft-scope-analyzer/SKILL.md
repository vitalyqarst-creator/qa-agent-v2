...
5. При `agent-proposed-scope` сохрани options/prompts в `00-<container-slug>/`; установи `awaiting-user-scope-selection`, `next_skill: ft-scope-analyzer` и пустой `blocking_reasons`. В `scope-options.md` добавь компактное `Распределение требований ФТ`: каждый формальный код требования назначен одному candidate scope, `cross-scope` с владельцем test design или обоснованному `out-of-scope`. До выбора не создавай writer/scope-local/BA artifacts или `scope-contract.md`; scope-зависимую неизвестность укажи только как риск option. Исключение — source contradiction, исключающее безопасное разбиение.
