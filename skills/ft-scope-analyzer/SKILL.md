...
9. Если для основного ФТ доступны DOCX и PDF, создай `source-parity-check.md` по `source-parity-check-format.md`: сверяй коды требований, таблицы, строки, примечания и границы только выбранного scope.
   ...
   До создания `ui-calibration` gap отдельно сверь относящиеся к экрану рисунки самого ФТ, PDF и доступные visual-only inputs. Если источник уже задаёт идентичность, название или расположение контрола, перенеси это в `visual_binding` связанного OBL; не называй такой вопрос UI-калибровкой. Для настоящей runtime-неопределённости зафиксируй в gap `visual_evidence_check` с проверенными источниками и остаточным вопросом.
