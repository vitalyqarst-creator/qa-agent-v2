...
Эти роли могут выполняться одним skill-ом, но reviewer обязан мыслить их как разные passes. Не закрывай дефект test-design только потому, что traceability формально есть, и не закрывай traceability только потому, что тест-кейс выглядит разумно.

## Режимы review

- `full` — канонический режим по умолчанию для direct review существующего набора. Выполняет `traceability`, затем `structure`, затем `test-design`, и возвращает общий findings artifact. Если требуется проверка покрытия, дополнительно возвращает traceability matrix. Direct `full` не заменяет session-based sign-off: для lifecycle `signed-off` используй отдельные stages `structure_preflight -> semantic_traceability_test_design -> structure_format_final -> semantic_regression` или `ft-test-case-iteration`.
...
- `structure_format_final` — финальная проверка оформления после semantic closure: шаблон, группировка, сквозная нумерация, wording и format smells.
...
13. В режиме `structure` или `full` проверь единый шаблон тест-кейса, корректность обязательных секций, группировку по функциональности/блоку/элементу/операции, сквозную нумерацию `TC-*` без пропусков/дублей/перезапуска внутри групп, порядок `позитивные -> негативные`, детерминированность шагов, отсутствие проверяемого действия в предусловиях, а также базовые проверки `видимость / редактируемость / обязательность`, если эти свойства явно описаны в ФТ.
...
15. Обязательно прогони defect-class checklist из `test-design-review-rubric.md` и `test-design-defect-taxonomy.md`: fake internal coverage, compact draft fallback, source table pollution, неатомарные `TC-*`, positive+negative в одном `TC-*`, action in preconditions, потерянные requirement codes, missing applicability/dependency/risk matrix, unsupported expected results, unsupported UI mechanism, mask covered by numeric-only, dictionary closed-set gaps, false/overbroad gaps, metadata-only behavior, date-window gap, exact-length gap, numeric-only class gap, repeatable-block lifecycle gap, action-created optionality gap, checkbox-list gap, print-form mapping gap, missing coverage metrics, negative transition without valid fixture, generic valid fixture/test-data oracle, applicability-linked TC drift, applicability atom contamination.
...
23. Во втором review сверяй не только обновленный набор тест-кейсов, но и writer response artifact.
...
28. Если набор подписан без unresolved findings, перед handoff убедись, что группировка и сквозная нумерация `TC-*` финализированы, заполни `Reviewer Sign-off Self-check` по `reviewer-output-format.md`, затем сохрани `prompt.reviewer-to-ui-prep.md` и обнови handoff state для следующего этапа.
