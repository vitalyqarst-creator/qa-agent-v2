...
До оценки строк matrix проверь входной inventory: один `SR-*` должен соответствовать одной обязанности, все действия из таблиц должны содержать ссылку на таблицу/строку, а отменённые утверждённым ответом требования должны находиться в разделе применённых исключений и отсутствовать среди активных `SR-*`. Убедись, что visual cross-check охватывает каждый включённый UI-уровень.
