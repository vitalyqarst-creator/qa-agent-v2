...
- `source-scope.md`, `scope-clarification-requests.md` и, при наличии,
  `test-data-source-plan.json` с `fixture-catalog.md`;
...
2. Для matrix проверь, что каждое независимое утверждение ФТ и каждый код из
   ...
   `not_checked`; известный `node-id` необязателен. При
   `visual-check-failed` проверь fallback на ФТ/PDF/локальный макет и то, что
   Figma-only UI-факт честно оставлен `BAQ-*` или
   `candidate-ui-calibration`. Подтверждённые Figma подписи и расположение
   могут конкретизировать UI-шаги, но не бизнес-правила.
   Для data-dependent строк проверь plan/catalog до verdict: literal должен
   иметь точный источник, provider-backed свойство — component path и
   verified value, file — формат и размер в байтах. Если provider не умеет
   доказать нужную семантику, требуй `BAQ-*`/`GAP-*`, а не аналогичное поле.
...
В обоих режимах используй `source-scope.md` как ограниченный индекс: один раз
проверь связанные source locators, глобальные правила и plan данных; не
перезапускай полное извлечение DOCX/PDF и не выполняй live-вызовы provider-а.
Расширяй выборку только при cross-reference или реальном признаке
противоречия. Это сокращает повторное чтение, но не отменяет независимую
проверку каждого связанного требования.
