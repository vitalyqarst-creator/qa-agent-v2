...
6. Если создаешь initial `automation-ready`, делай это как отдельную фазу:
   ...
   - если `Предусловия` требуют UI state без setup steps, fixture/API setup или reusable setup profile, пометь кейс как not automation-ready до уточнения setup path; не додумывай путь подготовки.
...
9. Для каждого кейса проставь один `ui_verification_status`:
   ...
   A test case is not automation-ready when `Предусловия` require a UI state but do not provide setup steps, fixture, API setup, or reusable setup profile.
