...
Before any prepared live writer, require the runner's observable-oracle preflight. If a terminal prior cycle has a complete unsigned draft and only allowlisted deterministic execution-oracle findings, recovery may compile one new immutable package/cycle and use the exec runner's hash-bound targeted-repair route. Pass the prior draft/findings only as unsigned repair input and repair only the named TC subset in one fresh read-only writer session. The runner may migrate only per-TC `package_id` metadata to the new package id; require exact byte evidence plus semantic preservation of every non-target section after normalizing that field, then run the ordinary full-set gates and one fresh reviewer. Never retry/resume the terminal cycle or treat its draft as requirement evidence. Before reviewer launch, require the package-metadata gate and, for every referenced `DICT-*`, the structured dictionary projection with exact active values.
