...
- One `SCN-*` normally has one TC and one primary expected result; apply the narrow semantic-duplicate exception from the canonical route. Different create/edit flows are separate CTX. One OBL/CTX may have several SCN only for independent source-backed classes, boundaries or initial states; do not duplicate one check.
