...
- Matrix declares initial state, its formation and the checked action separately. Test data are concrete values/files or precise properties plus a preparation method; never restate the checked rule as data. Put every trigger-state action in its own numbered step and add a follow-up observation when checking absence of persistence. For a one-file limit, attach an allowed file before attempting a distinct second file. If FT prescribes exact message text, reproduce it verbatim in the expected result. Mark a source-only internal action without an observable oracle as `blocked-observability`; do not invent an observation.
