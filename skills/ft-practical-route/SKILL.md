...
- `CTX-*` описывает поток и не заменяет вход на экран. Если способ навигации подтверждён источником или visual binding, укажи конкретное действие с контролом; если он не определён, зафиксируй уже открытый экран в предусловиях и соответствующий `SETUP-NAV-*`, не пиши шаг «Открыть ... в контексте ...». Для DaData-сценария используй сохранённый `FX-DADATA-*` с нужными literals. При его действительном отсутствии укажи точные свойства нужного ответа и отдельную строку `Способ подготовки: ...`; «организация с известными реквизитами» или «подготовить организацию» не являются данными.
