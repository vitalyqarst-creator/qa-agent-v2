...
2. Всегда создай или сохрани файл вопросов БА по
   [формату practical v1](../../references/agent/practical-v1-clarification-requests-format.md);
   не задерживай им работу с однозначными требованиями и не перезаписывай
   текст, внесённый БА.
3. Создай русскоязычную matrix и выполни self-check покрытия/атомарности.
   До review materialize test-data plan и fixture catalog, если строки зависят
   от provider-а, файла, синтетического значения или состояния среды; точное
   свойство provider-а должно быть доказано, а не выведено по смыслу.
4. Проведи independent matrix review в отдельной верхнеуровневой Codex-сессии.
   ...
   закрытия findings; один узкий `micro-closure` допустим только по контракту
   practical route v1.
...
6. Проведи independent TC review в отдельной верхнеуровневой Codex-сессии.
   ...
   закрытия findings. Новый или содержательный второй finding даёт
   `review-failed`, а не новый цикл.
...
- `needs-test-data` не оправдывает placeholders в draft TC: к нему всегда
  привязаны точный `TDP-*`, свойства подготовки и причина недоступности.
