...
| `DEC-004` | 4 | `validation` | runner timeout recovery | Preserve terminal `blocked-input`; do not treat draft as signed-off. | Writer session timed out and current-stage scoped validator profile was missing, so runner correctly refused reviewer handoff. | `blocked-runner-report.md`; `iteration-process-report.md` | high | applied |
