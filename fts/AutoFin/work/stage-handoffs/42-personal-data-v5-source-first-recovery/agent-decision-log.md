...
| `DEC-007` | 7 | `routing` | Единственный writer вернул capacity-related `blocked-input`; draft отсутствует | Остановить V5 без retry и маршрутизировать следующую итерацию на output-capacity preflight/alternative transport | Повтор того же one-shot контракта не меняет причину и нарушает live quota | `live-blocker-analysis.md`; `stop-gate.md` | high | applied |
