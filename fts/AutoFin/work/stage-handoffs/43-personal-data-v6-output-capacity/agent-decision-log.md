...
| `DEC-008` | 8 | `validation` | Checkpoint `ee4647c6757a70a24fda228ef88c1fa9651410b5` отправлен в origin; V6 attempts и production shadow отсутствуют | Разрешить ровно один V6 dispatcher после фиксации отдельного authorization artifact | Проверяемая git-граница отделяет validated implementation от необратимой live-попытки | `pre-live-authorization.md` | high | applied |
| `DEC-009` | 9 | `validation` | Один V6 dispatcher создал 4 fresh writer sessions и merged draft; quality gate нашёл 4 findings | Завершить V6 как `blocked-quality-gate`; не запускать reviewer, retry или resume | Stop gate выполнен; unsigned draft не прошёл исполнимость oracle | `live-result.v6.json`; V6 cycle state | high | applied |
| `DEC-010` | 10 | `source-boundary` | V6 merged draft покрывает 65/65 obligations, но четыре секции требуют ремонта | Разрешить successor использовать draft только как unsigned repair input, не как requirement evidence | Это сохраняет 43 пригодные секции без загрязнения source truth generated output-ом | `live-blocker-analysis.md`; successor prompt | high | applied |
| `DEC-011` | 11 | `routing` | Повтор четырёх full writer shards затратит около 121k tokens; finding IDs и TC IDs детерминированы | Следующий route — одна hash-bound targeted repair session, full-set gates и затем один reviewer | Минимальный объём повторной генерации при сохранении независимого semantic review | `prompt.scope-to-iteration.md` | high | proposed |
