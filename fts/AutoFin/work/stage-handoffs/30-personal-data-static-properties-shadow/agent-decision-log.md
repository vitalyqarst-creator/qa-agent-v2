...
| `DEC-008` | 8 | `validation` | V3 reviewer found overlap while literal diagnostic returned clean | Normalize generic value-type modifiers before signature comparison | Literal equality was too weak; reviewer evidence named the exact false negative | runner diagnostic v1; regression test | high | applied |
| `DEC-009` | 9 | `test-design` | V4 diagnostic and reviewer confirmed three groups | Group editability and string-value obligations by one planned TC per field | One action and one observable result should not create two executable cases | compiler inputs; `planned_test_case_id` | high | applied |
| `DEC-010` | 10 | `compatibility` | Existing immutable package v5 must remain readable | Add `planned_test_case_id` as optional package field | Enables grouping without invalidating historical v5 packages | `prepared_package.py`; canonical reference | medium | applied |
| `DEC-011` | 11 | `validation` | V5 structural validator rejected non-contiguous ids | Renumber grouped plan to contiguous `TC-PDSP-001..012` and create V6 | Structural gate is correct; bypassing it would weaken output quality | compiler inputs; V6 package | high | applied |
| `DEC-012` | 12 | `routing` | V6 reviewer accepted 12 TC / 15 obligations with clean overlap diagnostic | Complete shadow optimization and route next iteration to character restrictions | Requested process goals are proven without production write | `workflow-state.yaml`; prompt | high | applied |
