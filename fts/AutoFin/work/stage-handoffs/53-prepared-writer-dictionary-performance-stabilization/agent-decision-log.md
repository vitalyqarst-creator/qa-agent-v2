...
| `DEC-007` | 7 | `authorization` | User instruction and pushed offline checkpoint `c9dcbeb` | Разрешить r1 один раз; r2/r3 — по одному разу только после quality pass предыдущего run | Сохраняет независимые сессии, честный median и terminal stop discipline | `pre-live-authorization.v4.md` | high | applied |
| `DEC-008` | 8 | `validation` | r1 reviewer returned 12 covered + OBL-013 incorrect | Остановить V4; не запускать r2/r3 и не повторять r1 | Protocol treats any quality regression as terminal blocker | `terminal-stop-gate.v4.md` | high | applied |
| `DEC-009` | 9 | `architecture` | Exact OBL-013 values were present in writer prompt but absent from draft | Следующую итерацию направить на structured reference-only fixture transport и deterministic seed/gate | Prompt-only adherence уже оказался недостаточно надёжным | `prompt.v4-canary-to-reference-fixture-contract-v5.md` | high | proposed |
