...
| `DEC-008` | 8 | `live-gate` | Checkpoint `fc03115f...` is on origin; local/remote match; baseline unchanged | Authorize exactly one V8 dispatcher after authorization commit/push | All declared pre-live gates are satisfied and hash-bound | pre-live authorization | high | applied |
| `DEC-009` | 9 | `validation` | Reviewer returned valid `changes-required` for `TC-ACPD-011`; payload active_values=`[";"]` | Accept process transition but reject finding as source-backed draft defect | Canonical inventory and compiled evidence contain `Мужчина`; `Женщина`; projection parser extracted only separator | V8 findings/live blocker analysis | high | applied |
| `DEC-010` | 10 | `routing` | V8 is terminal and TC11 needs no semantic rewrite | Forbid retry/no-op repair; route only to new V9 parser remediation plus reviewer-only rebind | Repeating writer would waste tokens and mutate a source-correct draft | stop gate/successor prompt | high | applied |
