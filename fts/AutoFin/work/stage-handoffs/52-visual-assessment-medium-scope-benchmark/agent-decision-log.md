...
| `DEC-022` | 22 | `quality` | V2 reviewer returned F-001/F-002 after old dictionary/calibration defects closed | Close V2 without promotion or same-cycle retry | Semantic acceptance failed and one-shot authorization is consumed | V2 live result; stop gate | high | applied |
| `DEC-023` | 23 | `quality-feedback` | Both V2 defects make a user action ambiguous | Add compiler and runner action-unambiguity guards, then correct the fresh design plan | General pre-review prevention is preferable to manual draft repair | code/tests; V3 plan | high | applied |
| `DEC-024` | 24 | `execution` | V3 compiles and passes 462 tests, 61 architecture checks and validate-only | Prepare one final fresh-cycle checkpoint; keep live blocked pending separate authorization | Bounded rerun now tests one evidence-backed process change | V3 pre-live report/stop gate | high | applied |
| `DEC-025` | 25 | `execution` | Pushed remediation checkpoint `7e1c6ae...` matches origin | Authorize exactly one final V3 exec invocation and zero later live runs | Enforces the planned bounded-rerun limit | `pre-live-authorization.v3.md` | high | applied |
| `DEC-026` | 26 | `quality` | V3 reviewer accepted all 13 obligations with zero findings | Close V3 as accepted-not-promoted and preserve the candidate as benchmark evidence | Quality objective is achieved; promotion was outside the authorized benchmark | V3 live result; stop gate | high | applied |
| `DEC-027` | 27 | `performance` | V3 duration misses target by 4.532 s while overhead is 1.48% and uncached efficiency improves 23.96% vs V2 | Route next work to offline writer dictionary-deduplication and stable latency statistics | Single-run model latency is noisy; Python orchestration is not the bottleneck | performance analysis; next prompt | high | applied |
