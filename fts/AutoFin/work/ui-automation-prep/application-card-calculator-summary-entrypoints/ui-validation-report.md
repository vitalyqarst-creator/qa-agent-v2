...
UI-прогон состоялся и дал проверяемые локальные наблюдения, но фазу нельзя считать переносимо завершенной: два кейса заблокированы по observability, два выявили FT/UI divergence, безопасная воспроизводимая fixture отсутствует, а screenshots/snapshots/traces содержат персональные, application или session-sensitive данные и не могут быть опубликованы.
...
| `UI-F-004` | `high` | `TC-ACCS-001..005` | `evidence-quality-gap` | Evidence существует локально, но содержит PII/application data или session/payload risk. | UI evidence policy | `ui-evidence-index.md` | Публикация доказательств небезопасна; clean checkout не может независимо проверить статусы. | Повторно получить sanitized screenshots и trace на synthetic fixture без secrets/PII. | `yes` | `QA automation / Environment owner` |
...
- Portability verdict: `blocked` — ни один Playwright artifact не публикуется.
- Downstream обязан считать evidence `local-restricted-not-published`, а не доступным в clean checkout.
