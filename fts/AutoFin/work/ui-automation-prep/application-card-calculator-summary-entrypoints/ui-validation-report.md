...
| controlled_rerun | `not-run-gate-blocked` |
| normal_ui_path | `yes` |
...
| --- | --- | --- | --- |
| Safe reproducible fixture | `fail` | Runtime profile по-прежнему содержит только `LOCAL-RESTRICTED-FIXTURE-01`; create/select/reset runbook отсутствует. | Gate 1 не выполнен. |
| Normal UI calculator entrypoint | `fail` | Remediation diff не добавил новый runtime/setup artifact или подтверждение исправленного entrypoint. | Gate 2 не выполнен. |
| Independent TC2/TC5 observability | `fail` | Calculator stage/window остаются недоступны через документированный normal UI path. | Gate 3 не выполнен. |
| Safe evidence handling | `pass-with-limitation` | Restricted raw evidence допустимо оставить локально; safe index использует стабильные `UIE-*`. | Непубликация raw evidence сама по себе rerun не блокирует. |
| Product/FT-owner disposition | `unresolved` | Для `TC-ACCS-003` и `TC-ACCS-004` authoritative disposition недоступно. | Divergences явно остаются unresolved; это не используется как основание менять статусы. |

Вердикт: controlled UI rerun не запускался. Предыдущие статусы и evidence сохранены без изменений, потому что remediation-ветка устранила только часть evidence-contract debt, но не функциональные и fixture blockers.
...
- Для `TC-ACCS-003` и `TC-ACCS-004` отсутствует product/FT-owner disposition; divergences остаются unresolved, но rerun gate уже блокируется условиями fixture/observability.
...
- Focused validator policy/closeout tests: `5 passed`, including stable evidence IDs, explicit local limitations and completed-closeout blocker enforcement. The previously absent `tests/fixtures/agent-artifacts/ui-evidence-policy` fixture is present in the remediation source branch.
- Package-wide AutoFin validation still reports inherited unrelated debt: `5 errors`, `155 warnings`, `7 info`; active closeout paths have no error or warning.
- Script-first architecture audit: `0 errors`, `1 warning`; the only warning is inherited instruction-budget headroom for `reviewer.full_existing_cases` (`14.9 KiB` vs minimum `15.0 KiB`) outside this UI scope.
...
6. Исторический debt по отсутствующей fixture `tests/fixtures/agent-artifacts/ui-evidence-policy` снят в remediation source branch; focused validator tests теперь проходят.
