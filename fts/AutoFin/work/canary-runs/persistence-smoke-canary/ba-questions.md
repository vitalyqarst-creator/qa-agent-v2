...
| `BA-PS-005` | `blocking-for-execution` | Which application date must be used as `D` for contact-person birth-date calculations: current UI business date, server date, or tester local date? | `TC-AF43-PS-006` uses `D - 30 calendar years`; `TC-AF43-PS-007` uses a concrete setup date that must remain acceptable for the execution date. | `TC-AF43-PS-006`; `TC-AF43-PS-007` setup |
