...
- plan: PLAN-034 | atoms=ATOM-030 | check=Ввести `Петрова-Сидорова` в поле `Предыдущая фамилия` и завершить ввод. | expected=После ввода поле `Предыдущая фамилия` визуально содержит `Петрова-Сидорова`. | planned=TC-ACPD-034 | status=covered
