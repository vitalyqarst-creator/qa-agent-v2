...
{
  "package_version": 5,
  "package_id": "application-card-client-personal-data-v4r1",
  "ft_slug": "AutoFin",
...
- source_path: `fts/AutoFin/work/review-cycles/application-card-client-personal-data-shadow-v4r1-20260713/prepared-input/.application-card-client-personal-data-v4r1.compiled-evidence.md`
- source_sha256: `1bec8a253ee6abb9e04065eda25658668c74b0f9118c512844c6d9b0de37cb89`
...
- plan: PLAN-022 | atoms=ATOM-003 | check=По `FIX-ACPD-SAVE-001` оставить `Фамилия` пустой, повторить control-save, записать screenshot/post-state/persistence. | expected=Evidence: control/action/empty `Фамилия`/post-state/persistence; UI-механизм не предписывается. | planned=TC-ACPD-022 | status=covered
...
- plan: PLAN-023 | atoms=ATOM-008 | check=По `FIX-ACPD-SAVE-001` оставить `Имя` пустым, повторить control-save, записать screenshot/post-state/persistence. | expected=Evidence: control/action/empty `Имя`/post-state/persistence; UI-механизм не предписывается. | planned=TC-ACPD-023 | status=covered
...
- plan: PLAN-047 | atoms=ATOM-013 | check=По `FIX-ACPD-SAVE-001` оставить `Отчество` пустым, выполнить control-save, открыть заявку повторно. | expected=Save завершён; после повторного открытия `Отчество` пусто и не блокировало сохранение. | planned=TC-ACPD-047 | status=covered
...
- plan: PLAN-010 | atoms=ATOM-018 | check=По `FIX-ACPD-SAVE-001` записать пустой `ID клиента`, выполнить control-проверенное сохранение, записать новое видимое значение. | expected=Видимый `ID клиента` изменился с пустого на непустой; значение записано, ABS-атрибуция не утверждается. | planned=TC-ACPD-010 | status=covered
...
- plan: PLAN-024 | atoms=ATOM-019 | check=По `FIX-ACPD-SAVE-001` оставить `Пол` пустым, повторить control-save, записать screenshot/post-state/persistence. | expected=Evidence: control/action/empty `Пол`/post-state/persistence; UI-механизм не предписывается. | planned=TC-ACPD-024 | status=covered
...
- plan: PLAN-012 | atoms=ATOM-020 | check=По `FIX-ACPD-DADATA-001` записать исходный `Пол`, ввести зарегистрированное ФИО, выбрать записанную подсказку, сравнить `Пол` с expected fixture. | expected=Видимый `Пол` совпал с expected fixture; до/после записаны, provider-attribution не утверждается. | planned=TC-ACPD-012 | status=covered
...
- plan: PLAN-025 | atoms=ATOM-021 | check=По `FIX-ACPD-SAVE-001` оставить `Дата рождения` пустой, повторить control-save, записать screenshot/post-state/persistence. | expected=Evidence: control/action/empty `Дата рождения`/post-state/persistence; UI-механизм не предписывается. | planned=TC-ACPD-025 | status=covered
...
- plan: PLAN-041 | atoms=ATOM-031`; `ATOM-036`; `ATOM-041 | check=По `FIX-ACPD-SAVE-001` задать `Клиент менял ФИО=Да`, оставить previous-FIO пустыми, повторить control-save, записать screenshot/post-state/persistence. | expected=Evidence: control/action/condition/empty group/post-state/persistence; UI-механизм не предписывается. | planned=TC-ACPD-041 | status=covered
...
```json
{
  "artifact": "fts/AutoFin/work/review-cycles/application-card-client-personal-data-shadow-v4r1-20260713/prepared-input/application-card-client-personal-data-v4r1/atomic-obligations.json",
  "artifact_sha256": "ca8e23e12b8e4d36918f1deeca75a5b0f3de23d76e7a8cdb8ea720cb1a599d29",
  "obligation_count": 65,
