...
[SEED:observable oracle] После выбора runtime-подсказки в `Фамилия` видимый `Пол` равен полу выбранного варианта; значения записаны, provider-attribution не утверждается.
...
[SEED:observable oracle] После ввода вычисленной от `D` даты `D-18 лет` поле `Дата рождения` показывает ту же логическую дату.; Граница `D-18 лет` вычисляется относительно текущей даты приложения `D`.
...
[SEED:observable oracle] После ввода вычисленной от `D` даты `D-100 лет` поле `Дата рождения` показывает ту же логическую дату.; Граница `D-100 лет` вычисляется относительно текущей даты приложения `D`.
...
[SEED:observable oracle] Evidence record: `D-18 лет + 1 день`, граница `D-18 лет`, завершение ввода, сохранение, видимое состояние поля и outcome; UI-механизм не предписывается.; Проверка значения позже `D-18 лет` использует текущую дату приложения `D`.
...
[SEED:observable oracle] Evidence record: `D+1 день`, дата `D`, завершение ввода, сохранение, видимое состояние поля и outcome; UI-механизм не предписывается.; Проверка значения больше `D` использует текущую дату приложения `D`.
...
[SEED:observable oracle] Evidence record: `D-100 лет - 1 день`, граница `D-100 лет`, завершение ввода, сохранение, видимое состояние поля и outcome; UI-механизм не предписывается.; Проверка значения раньше `D-100 лет` использует текущую дату приложения `D`.
...
[SEED:observable oracle] После ввода поле `Предыдущая фамилия` визуально содержит `Петрова-Сидорова`.
