...
- Область: `4.3-current-passport-data`.
- Суть: follow-up UI evidence подтвердил DaData/FMS путь для поля `Кем выдан` с fixture `FX-DADATA-FMS-POS-001`.
- Проверенные данные: `Код подразделения = 772-053`; dropdown item `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`; итоговое значение поля `Кем выдан = ОВД ЗЮЗИНО Г. МОСКВЫ`.
- Решение: считать `TC-PASSCUR-003`, `TC-PASSCUR-005` и `TC-PASSCUR-018` `confirmed` / automation-ready при явном trigger `click/focus` в поле `Кем выдан` после ввода кода подразделения.
- Ограничение: не ожидать автозаполнение поля `Кем выдан` сразу после ввода кода, blur или Enter; evidence показал, что dropdown появляется после focus/click в `Кем выдан`.
