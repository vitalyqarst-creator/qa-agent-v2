...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какое действие запускает проверку обязательности пустого поля Имя и какой фактический UI-отклик отображается?
...
2. Выполнить действие проверки/валидации для Имя: Проверить обязательность поля `Имя`.
3. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.
