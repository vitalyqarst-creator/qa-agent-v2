...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой фактический UI-отклик отображается при вводе цифры или неразрешённого спецсимвола в поле Имя?
...
1. Ввести `Иван-Петров` в Имя.
2. Проверить, что значение `Иван-Петров` соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
3. Очистить Имя после проверки `Иван-Петров`.
4. Ввести `Иван-Петров1` в Имя.
5. Проверить, что значение `Иван-Петров1` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
6. Очистить Имя после проверки `Иван-Петров1`.
7. Ввести `Иван-Петров@` в Имя.
8. Проверить, что значение `Иван-Петров@` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
9. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой фактический UI-отклик отображается при вводе цифры или неразрешённого спецсимвола в поле Фамилия?
...
1. Ввести `Иван-Петров` в Фамилия.
2. Проверить, что значение `Иван-Петров` соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
3. Очистить Фамилия после проверки `Иван-Петров`.
4. Ввести `Иван-Петров1` в Фамилия.
5. Проверить, что значение `Иван-Петров1` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
6. Очистить Фамилия после проверки `Иван-Петров1`.
7. Ввести `Иван-Петров@` в Фамилия.
8. Проверить, что значение `Иван-Петров@` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
9. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какое действие запускает проверку обязательности пустого поля Фамилия и какой фактический UI-отклик отображается?
...
1. Оставить Фамилия пустым.
2. Выполнить действие проверки/валидации для Фамилия: Проверить обязательность поля `Фамилия`.
3. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой фактический UI-отклик отображается при вводе цифры или неразрешённого спецсимвола в поле Отчество?
...
1. Ввести `Иван-Петров` в Отчество.
2. Проверить, что значение `Иван-Петров` соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
3. Очистить Отчество после проверки `Иван-Петров`.
4. Ввести `Иван-Петров1` в Отчество.
5. Проверить, что значение `Иван-Петров1` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
6. Очистить Отчество после проверки `Иван-Петров1`.
7. Ввести `Иван-Петров@` в Отчество.
8. Проверить, что значение `Иван-Петров@` не соответствует правилу: Допускаются только текстовые символы и специальный символ `-`.
9. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какое действие подтверждает необязательность пустого поля Отчество и какой фактический UI-отклик отображается?
...
1. Оставить Отчество пустым.
2. Выполнить действие проверки/валидации для Отчество: Проверить обязательность поля `Отчество`.
3. Завершить ввод в целевом поле или инициировать доступную точку проверки без выбора конкретного механизма отклонения/подтверждения.


### Итоговый ожидаемый результат

Пустое значение необязательного поля не должно считаться нарушением требования ФТ. Конкретный наблюдаемый механизм подтверждения необязательности требуется зафиксировать при UI calibration: успешное завершение выбранного действия без ошибки / отсутствие маркера обязательности / отсутствие сообщения / другое фактическое поведение.
