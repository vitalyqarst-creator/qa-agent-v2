# FT-first baseline: 4-3-current-passport-data

> Финальный статус: `reviewed-with-known-findings`.
> Набор создан из shadow draft attempt `20260727-4-3-current-passport-data-source-first-attempt-021` после passed production suite gate, но reviewer verdict = `changes-required`.
> Это не `signed-off` и не production-ready sign-off. Известный дефект покрытия: `SRC-PASS-CUR-010` / `OBL-PASS-CUR-027` не материализован как отдельная obligation/case.
> Подробности: `fts/AutoFin/PostFinal-v2-rc5/work/iterations/20260727-4-3-current-passport-data-source-first-attempt-021/finalization-receipt.json`.

# Тест-кейсы: Карточка «Заявка» / Блок «Паспортные данные»

## TC-PASSCUR-001

**№:** 1
**Название:** Поле ручного ввода «Кем выдан» обязательно, если «Ввести вручную подразделение» = «Да»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-033; ATOM-033; ASSERT-PASS-CUR-033
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Кем выдан: оставить пустым.

### Шаги

1. Установить переключатель «Ввести вручную подразделение» в значение «Да».
2. Оставить поле «Кем выдан» пустым.
3. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Выберите значение».

### Постусловия

Не требуются.

## TC-PASSCUR-002

**№:** 2
**Название:** Поле «Кем выдан» в режиме DaData обязательно, если «Ввести вручную подразделение» = «Нет»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-032; ATOM-032; ASSERT-PASS-CUR-032
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Кем выдан: оставить пустым.

### Шаги

1. Установить переключатель «Ввести вручную подразделение» в значение «Нет».
2. Оставить поле «Кем выдан» пустым.
3. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Выберите значение».

### Постусловия

Не требуются.

## TC-PASSCUR-003

**№:** 3
**Название:** Поле «Кем выдан» предзаполняется по коду подразделения
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-015; ATOM-015; ASSERT-PASS-CUR-015; BSR 94
**UI status:** confirmed
**Automation note:** DaData/FMS поведение подтверждено fixture `FX-DADATA-FMS-POS-001`: код `772-053`, focus/click `Кем выдан`, выбор `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`, итоговое значение `ОВД ЗЮЗИНО Г. МОСКВЫ`.
**Evidence:** `work/ui-automation-prep/passcur-dadata-fms-772-053/ui-validation-report.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Fixture: `FX-DADATA-FMS-POS-001`.
- Запрос / код подразделения: `772-053`.
- Точное предложение в UI: `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.
- Ожидаемое значение поля `Кем выдан` после выбора: `ОВД ЗЮЗИНО Г. МОСКВЫ`.

### Шаги

1. Установить переключатель `Ввести вручную подразделение` в значение `Нет`.
2. Ввести `772-053` в поле `Код подразделения`.
3. Нажать/сфокусировать поле `Кем выдан`.
4. Дождаться выпадающего списка и найти предложение `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.
5. Выбрать предложение `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.
6. Проверить значение поля `Кем выдан`.

### Итоговый ожидаемый результат

После focus/click в поле `Кем выдан` отображается предложение `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`. После выбора поле `Кем выдан` содержит `ОВД ЗЮЗИНО Г. МОСКВЫ`.

### Постусловия

Не требуются.

## TC-PASSCUR-004

**№:** 4
**Название:** Отображение поля ручного ввода «Кем выдан» при включенном ручном вводе
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-019; ATOM-019; ASSERT-PASS-CUR-019; BSR 98
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Установить переключатель «Ввести вручную подразделение» в значение «Да».
2. Проверить отображение поля «Кем выдан».

### Итоговый ожидаемый результат

Отображается текстовое поле «Кем выдан» для ручного ввода.

### Постусловия

Не требуются.

## TC-PASSCUR-005

**№:** 5
**Название:** Отображение списка «Кем выдан» при выключенном ручном вводе
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-014; ATOM-014; ASSERT-PASS-CUR-014; BSR 93
**UI status:** confirmed
**Automation note:** DaData/FMS autocomplete подтверждён fixture `FX-DADATA-FMS-POS-001`: список появляется после focus/click в `Кем выдан`, не сразу после blur/Enter из `Код подразделения`.
**Evidence:** `work/ui-automation-prep/passcur-dadata-fms-772-053/ui-validation-report.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Fixture: `FX-DADATA-FMS-POS-001`.
- Код подразделения: `772-053`.
- Ожидаемое предложение: `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.

### Шаги

1. Установить переключатель `Ввести вручную подразделение` в значение `Нет`.
2. Ввести `772-053` в поле `Код подразделения`.
3. Нажать/сфокусировать поле `Кем выдан`.
4. Проверить, что открылся список предложений DaData/FMS.

### Итоговый ожидаемый результат

Поле `Кем выдан` работает как selectable/autocomplete control. Список предложений появляется после focus/click в поле `Кем выдан` и содержит `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.

### Постусловия

Не требуются.

## TC-PASSCUR-006

**№:** 6
**Название:** Значение по умолчанию переключателя «Клиент менял паспорт»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-028; ATOM-028; ASSERT-PASS-CUR-028; BSR 104
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Ожидаемое значение: `Нет`.

### Шаги

1. Не взаимодействуя с переключателем «Клиент менял паспорт», проверить его первоначальное значение `Нет`.

### Итоговый ожидаемый результат

Переключатель «Клиент менял паспорт» имеет значение «Нет».

### Постусловия

Не требуются.

## TC-PASSCUR-007

**№:** 7
**Название:** Отображение переключателя «Клиент менял паспорт» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-027; ATOM-027; ASSERT-PASS-CUR-027; BSR 103
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение переключателя «Клиент менял паспорт».

### Итоговый ожидаемый результат

Переключатель «Клиент менял паспорт» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-008

**№:** 8
**Название:** Поле «Место рождения» обязательно для заполнения
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-035; ATOM-035; ASSERT-PASS-CUR-035
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Место рождения: оставить пустым.

### Шаги

1. Оставить поле «Место рождения» пустым.
2. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Обязательно к заполнению».

### Постусловия

Не требуются.

## TC-PASSCUR-009

**№:** 9
**Название:** Отображение поля «Место рождения» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-026; ATOM-026; ASSERT-PASS-CUR-026; BSR 102
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение поля «Место рождения».

### Итоговый ожидаемый результат

Поле «Место рождения» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-010

**№:** 10
**Название:** Нечисловые символы в поле «Номер»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-008; ATOM-008; ASSERT-PASS-CUR-008; BSR 88
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для нечислового значения номера и required-style validation `Обязательно к заполнению`.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Нечисловое значение: `123A56`.

### Шаги

1. Ввести `123A56` в поле `Номер`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Нечисловое значение очищается после blur. Поле `Номер` становится пустым и показывает required-style validation `Обязательно к заполнению`.

### Постусловия

Не требуются.

## TC-PASSCUR-011

**№:** 11
**Название:** Ввод более шести цифр в поле «Номер»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-007; ATOM-007; ASSERT-PASS-CUR-007; BSR 88
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение: `1234567`.

### Шаги

1. Ввести `1234567` в поле «Номер».
2. Проверить отображаемое значение поля «Номер».
3. Очистить поле «Номер».

### Итоговый ожидаемый результат

Седьмой символ не вводится; в поле остается значение `123456` длиной 6 символов.

### Постусловия

Не требуются.

## TC-PASSCUR-012

**№:** 12
**Название:** Короткое значение в поле «Номер»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-007; ATOM-007; ASSERT-PASS-CUR-007; BSR 88; SO-CAL-B9AA3D5996B08EBD
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для короткого номера `12345` и сообщение `Обязательно к заполнению`.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-012.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Короткое значение номера: `12345`.

### Шаги

1. Ввести `12345` в поле `Номер`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Значение `12345` очищается после blur. Поле `Номер` становится пустым и показывает сообщение `Обязательно к заполнению`.

### Постусловия

Не требуются.

## TC-PASSCUR-013

**№:** 13
**Название:** Шестизначный номер паспорта
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-007; ATOM-007; ASSERT-PASS-CUR-007; BSR 88
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Допустимое значение: `123456`.

### Шаги

1. Ввести `123456` в поле «Номер».
2. Проверить отображаемое значение поля «Номер».
3. Очистить поле «Номер».

### Итоговый ожидаемый результат

Поле «Номер» отображает значение `123456`.

### Постусловия

Не требуются.

## TC-PASSCUR-014

**№:** 14
**Название:** Шесть одинаковых цифр подряд в поле «Номер»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-009; ATOM-009; ASSERT-PASS-CUR-009; BSR 89
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение: `111111`.

### Шаги

1. Ввести `111111` в поле «Номер».
2. Проверить состояние поля и сообщение под ним.

### Итоговый ожидаемый результат

Значение `111111` вводится, поле подсвечивается красным, под полем отображается сообщение «Не должно быть шести одинаковых цифр подряд».

### Постусловия

Не требуются.

## TC-PASSCUR-015

**№:** 15
**Название:** Поле «Номер» обязательно для заполнения
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-030; ATOM-030; ASSERT-PASS-CUR-030
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Номер: оставить пустым.

### Шаги

1. Оставить поле «Номер» пустым.
2. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Обязательно к заполнению».

### Постусловия

Не требуются.

## TC-PASSCUR-016

**№:** 16
**Название:** Отображение поля «Номер» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-006; ATOM-006; ASSERT-PASS-CUR-006; BSR 87
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение поля «Номер».

### Итоговый ожидаемый результат

Поле «Номер» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-017

**№:** 17
**Название:** Значение по умолчанию переключателя «Ввести вручную подразделение»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-018; ATOM-018; ASSERT-PASS-CUR-018; BSR 97
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Ожидаемое значение: `Нет`.

### Шаги

1. Не взаимодействуя с переключателем «Ввести вручную подразделение», проверить его первоначальное значение `Нет`.

### Итоговый ожидаемый результат

Переключатель «Ввести вручную подразделение» имеет значение «Нет».

### Постусловия

Не требуются.

## TC-PASSCUR-018

**№:** 18
**Название:** Выбор значения «Кем выдан» из предложений DaData
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-016; ATOM-016; ASSERT-PASS-CUR-016; BSR 95
**UI status:** confirmed
**Automation note:** Сохранность выбранного DaData/FMS значения подтверждена fixture `FX-DADATA-FMS-POS-001`: после выбора и blur поле `Кем выдан` сохраняет `ОВД ЗЮЗИНО Г. МОСКВЫ`.
**Evidence:** `work/ui-automation-prep/passcur-dadata-fms-772-053/ui-validation-report.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Fixture: `FX-DADATA-FMS-POS-001`.
- Код подразделения: `772-053`.
- Выбираемое предложение: `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.
- Ожидаемое значение поля `Кем выдан`: `ОВД ЗЮЗИНО Г. МОСКВЫ`.

### Шаги

1. Установить переключатель `Ввести вручную подразделение` в значение `Нет`.
2. Ввести `772-053` в поле `Код подразделения`.
3. Нажать/сфокусировать поле `Кем выдан`.
4. Выбрать предложение `ОВД ЗЮЗИНО Г. МОСКВЫ 772-053`.
5. Вывести фокус из поля `Кем выдан`.
6. Проверить значение поля `Кем выдан` после blur/перехода.

### Итоговый ожидаемый результат

После выбора поле `Кем выдан` содержит `ОВД ЗЮЗИНО Г. МОСКВЫ`. После blur/перехода выбранное значение остаётся в поле.

### Постусловия

Не требуются.

## TC-PASSCUR-019

**№:** 19
**Название:** Отображение переключателя «Ввести вручную подразделение» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-017; ATOM-017; ASSERT-PASS-CUR-017; BSR 96
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение переключателя «Ввести вручную подразделение».

### Итоговый ожидаемый результат

Переключатель «Ввести вручную подразделение» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-020

**№:** 20
**Название:** Просрочка паспорта на 91-й день после 45-летия
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-023; ATOM-023; ASSERT-PASS-CUR-023; BSR 100
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое граничное условие: `дата выдачи = дата 45-летия - 1 день; текущая дата = дата 45-летия + 91 день`.

### Шаги

1. Подготовить условие `дата выдачи = дата 45-летия - 1 день; текущая дата = дата 45-летия + 91 день`.
2. Ввести рассчитанную дату выдачи в поле «Дата выдачи».
3. Инициировать проверку срока действия паспорта.

### Итоговый ожидаемый результат

Сохранение блокируется; отображается подсказка «Паспорт недействителен (просрочен)».

### Постусловия

Не требуются.

## TC-PASSCUR-021

**№:** 21
**Название:** Дата выдачи за день до 14-летия клиента
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-021; ATOM-021; ASSERT-PASS-CUR-021; BSR 100
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое граничное условие: `дата 14-летия - 1 день`.

### Шаги

1. Рассчитать дату по условию `дата 14-летия - 1 день`.
2. Ввести рассчитанную дату в поле «Дата выдачи».
3. Инициировать проверку сохранения.

### Итоговый ожидаемый результат

Сохранение блокируется с подсказкой «Выдача паспорта предусмотрена с 14 лет».

### Постусловия

Не требуются.

## TC-PASSCUR-022

**№:** 22
**Название:** Просрочка паспорта на 91-й день после 20-летия
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-022; ATOM-022; ASSERT-PASS-CUR-022; BSR 100
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое граничное условие: `дата выдачи = дата 20-летия; текущая дата = дата 20-летия + 91 день`.

### Шаги

1. Подготовить условие `дата выдачи = дата 20-летия; текущая дата = дата 20-летия + 91 день`.
2. Ввести рассчитанную дату выдачи в поле «Дата выдачи».
3. Инициировать проверку срока действия паспорта.

### Итоговый ожидаемый результат

Сохранение блокируется; отображается подсказка «Паспорт недействителен (просрочен)».

### Постусловия

Не требуются.

## TC-PASSCUR-023

**№:** 23
**Название:** Срок действия паспорта на 90-й день после 45-летия
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-023; ATOM-023; ASSERT-PASS-CUR-023; BSR 100; SO-CAL-130A52DD73C6B7BF
**UI status:** confirmed
**Automation note:** UI evidence подтверждает field-level valid state: дата выдачи остаётся заполненной, контейнер valid, видимое сообщение отсутствует. Даты рассчитывать динамически от даты запуска.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-023.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- `D` — текущая дата выполнения теста.
- 45-летие клиента: `D - 90 дней`.
- Дата рождения клиента: `(D - 90 дней) - 45 лет`.
- Дата выдачи паспорта: `(D - 90 дней) - 1 день`.
- Пример из UI evidence для `D = 28.07.2026`: дата рождения `29.04.1981`, дата выдачи `28.04.2026`.

### Шаги

1. Рассчитать дату рождения и дату выдачи по формулам из тестовых данных.
2. Ввести рассчитанную дату рождения в поле `Дата рождения`.
3. Ввести рассчитанную дату выдачи в поле `Дата выдачи`.
4. Вывести фокус из поля `Дата выдачи`.
5. Проверить состояние поля `Дата выдачи`.

### Итоговый ожидаемый результат

Поле `Дата выдачи` сохраняет введённую дату, находится в valid state, видимое сообщение об ошибке отсутствует.

### Постусловия

Не требуются.

## TC-PASSCUR-024

**№:** 24
**Название:** Дата выдачи в день 14-летия клиента
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-021; ATOM-021; ASSERT-PASS-CUR-021; BSR 100; SO-CAL-E157E14D70EE210D
**UI status:** confirmed
**Automation note:** UI evidence подтверждает field-level valid state для даты выдачи в день 14-летия: значение остаётся, invalid class и видимое сообщение отсутствуют.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-024.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- `D` — текущая дата выполнения теста.
- Дата рождения клиента: `D - 14 лет`.
- Дата выдачи паспорта: `D` — дата 14-летия клиента.
- Пример из UI evidence для `D = 28.07.2026`: дата рождения `28.07.2012`, дата выдачи `28.07.2026`.

### Шаги

1. Рассчитать дату рождения и дату выдачи по формулам из тестовых данных.
2. Ввести рассчитанную дату рождения в поле `Дата рождения`.
3. Ввести рассчитанную дату выдачи в поле `Дата выдачи`.
4. Вывести фокус из поля `Дата выдачи`.
5. Проверить состояние поля `Дата выдачи`.

### Итоговый ожидаемый результат

Поле `Дата выдачи` сохраняет введённую дату; invalid class и видимое сообщение об ошибке отсутствуют.

### Постусловия

Не требуются.

## TC-PASSCUR-025

**№:** 25
**Название:** Срок действия паспорта на 90-й день после 20-летия
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-022; ATOM-022; ASSERT-PASS-CUR-022; BSR 100; SO-CAL-1E45C304CD480613
**UI status:** confirmed
**Automation note:** UI evidence подтверждает field-level valid state: дата выдачи остаётся заполненной, контейнер valid, видимое сообщение отсутствует. Даты рассчитывать динамически от даты запуска.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-025.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- `D` — текущая дата выполнения теста.
- 20-летие клиента: `D - 90 дней`.
- Дата рождения клиента: `(D - 90 дней) - 20 лет`.
- Дата выдачи паспорта: `D - 90 дней`.
- Пример из UI evidence для `D = 28.07.2026`: дата рождения `29.04.2006`, дата выдачи `29.04.2026`.

### Шаги

1. Рассчитать дату рождения и дату выдачи по формулам из тестовых данных.
2. Ввести рассчитанную дату рождения в поле `Дата рождения`.
3. Ввести рассчитанную дату выдачи в поле `Дата выдачи`.
4. Вывести фокус из поля `Дата выдачи`.
5. Проверить состояние поля `Дата выдачи`.

### Итоговый ожидаемый результат

Поле `Дата выдачи` сохраняет введённую дату, находится в valid state, видимое сообщение об ошибке отсутствует.

### Постусловия

Не требуются.

## TC-PASSCUR-026

**№:** 26
**Название:** Дата выдачи паспорта в день 45-летия
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-024; ATOM-024; ASSERT-PASS-CUR-024; BSR 100; SO-CAL-1E93A6C34ABC8C9C
**UI status:** confirmed
**Automation note:** UI evidence подтверждает field-level valid state для даты выдачи в день 45-летия: значение остаётся, контейнер valid, видимое сообщение отсутствует.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-026.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- `D` — текущая дата выполнения теста.
- Дата рождения клиента: `D - 45 лет`.
- Дата выдачи паспорта: `D` — дата 45-летия клиента.
- Пример из UI evidence для `D = 28.07.2026`: дата рождения `28.07.1981`, дата выдачи `28.07.2026`.

### Шаги

1. Рассчитать дату рождения и дату выдачи по формулам из тестовых данных.
2. Ввести рассчитанную дату рождения в поле `Дата рождения`.
3. Ввести рассчитанную дату выдачи в поле `Дата выдачи`.
4. Вывести фокус из поля `Дата выдачи`.
5. Проверить состояние поля `Дата выдачи`.

### Итоговый ожидаемый результат

Поле `Дата выдачи` сохраняет введённую дату, находится в valid state, видимое сообщение об ошибке отсутствует.

### Постусловия

Не требуются.

## TC-PASSCUR-027

**№:** 27
**Название:** Дата выдачи позже текущей даты
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-025; ATOM-025; ASSERT-PASS-CUR-025; BSR 101
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение больше текущей даты: `текущая дата + 1 день`.

### Шаги

1. Рассчитать дату `текущая дата + 1 день` относительно даты выполнения проверки.
2. Ввести рассчитанное значение `текущая дата + 1 день` в поле «Дата выдачи».
3. Проверить сообщение под полем.
4. Очистить поле «Дата выдачи».

### Итоговый ожидаемый результат

Под полем отображается текст «Дата не может быть больше <текущая дата в формате ДД.ММ.ГГГГ>».

### Постусловия

Не требуются.

## TC-PASSCUR-028

**№:** 28
**Название:** Допустимые граничные значения поля «Дата выдачи»
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-025; ATOM-025; ASSERT-PASS-CUR-025; BSR 101
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Допустимое граничное значение: `текущая дата - 1 день`.
- Допустимое граничное значение: `текущая дата`.

### Шаги

1. Рассчитать значения `текущая дата - 1 день` и `текущая дата` относительно даты выполнения проверки.
2. Ввести `текущая дата - 1 день` в поле «Дата выдачи».
3. Проверить, что поле «Дата выдачи» отображает рассчитанное значение `текущая дата - 1 день`.
4. Очистить поле «Дата выдачи».
5. Ввести `текущая дата` в поле «Дата выдачи».
6. Проверить, что поле «Дата выдачи» отображает рассчитанное значение `текущая дата`.
7. Очистить поле «Дата выдачи».

### Итоговый ожидаемый результат

Поле «Дата выдачи» отображает рассчитанное значение после ввода каждой из дат: `текущая дата - 1 день` и `текущая дата`.

### Постусловия

Не требуются.

## TC-PASSCUR-029

**№:** 29
**Название:** Поле «Дата выдачи» обязательно для заполнения
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-034; ATOM-034; ASSERT-PASS-CUR-034
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Дата выдачи: оставить пустым.

### Шаги

1. Оставить поле «Дата выдачи» пустым.
2. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Введите дату».

### Постусловия

Не требуются.

## TC-PASSCUR-030

**№:** 30
**Название:** Отображение поля «Дата выдачи» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-020; ATOM-020; ASSERT-PASS-CUR-020; BSR 99
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение поля «Дата выдачи».

### Итоговый ожидаемый результат

Поле «Дата выдачи» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-031

**№:** 31
**Название:** Нечисловые символы в поле «Серия»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-004; ATOM-004; ASSERT-PASS-CUR-004; BSR 85
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для нечислового значения серии и required-style validation `Обязательно к заполнению`.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Нечисловое значение серии: `12A4`.

### Шаги

1. Ввести `12A4` в поле `Серия`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Нечисловое значение очищается после blur. Поле `Серия` становится пустым и показывает required-style validation `Обязательно к заполнению`.

### Постусловия

Не требуются.

## TC-PASSCUR-032

**№:** 32
**Название:** Ввод более четырех цифр в поле «Серия»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-003; ATOM-003; ASSERT-PASS-CUR-003; BSR 85
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение: `12345`.

### Шаги

1. Ввести `12345` в поле «Серия».
2. Проверить отображаемое значение поля «Серия».
3. Очистить поле «Серия».

### Итоговый ожидаемый результат

Пятый символ не вводится; в поле остается значение `1234` длиной 4 символа.

### Постусловия

Не требуются.

## TC-PASSCUR-033

**№:** 33
**Название:** Короткое значение в поле «Серия»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-003; ATOM-003; ASSERT-PASS-CUR-003; BSR 85; SO-CAL-B1B43294C3EFD8EC
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для короткой серии `123` и сообщение `Обязательно к заполнению`.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-033.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Короткое значение серии: `123`.

### Шаги

1. Ввести `123` в поле `Серия`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Значение `123` очищается после blur. Поле `Серия` становится пустым и показывает сообщение `Обязательно к заполнению`.

### Постусловия

Не требуются.

## TC-PASSCUR-034

**№:** 34
**Название:** Четырехзначная серия паспорта
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-003; ATOM-003; ASSERT-PASS-CUR-003; BSR 85
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Допустимое значение: `1234`.

### Шаги

1. Ввести `1234` в поле «Серия».
2. Проверить отображаемое значение поля «Серия».
3. Очистить поле «Серия».

### Итоговый ожидаемый результат

Поле «Серия» отображает значение `1234`.

### Постусловия

Не требуются.

## TC-PASSCUR-035

**№:** 35
**Название:** Три одинаковые цифры подряд в поле «Серия»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-005; ATOM-005; ASSERT-PASS-CUR-005; BSR 86
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение: `111`.

### Шаги

1. Ввести `111` в поле «Серия».
2. Проверить состояние поля и сообщение под ним.

### Итоговый ожидаемый результат

Значение `111` вводится, поле подсвечивается красным, под полем отображается сообщение «Не должно быть трех одинаковых цифр подряд».

### Постусловия

Не требуются.

## TC-PASSCUR-036

**№:** 36
**Название:** Поле «Серия» обязательно для заполнения
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-029; ATOM-029; ASSERT-PASS-CUR-029
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Серия: оставить пустым.

### Шаги

1. Оставить поле «Серия» пустым.
2. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Обязательно к заполнению».

### Постусловия

Не требуются.

## TC-PASSCUR-037

**№:** 37
**Название:** Отображение поля «Серия» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-002; ATOM-002; ASSERT-PASS-CUR-002; BSR 84
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение поля «Серия».

### Итоговый ожидаемый результат

Поле «Серия» отображается.

### Постусловия

Не требуются.

## TC-PASSCUR-038

**№:** 38
**Название:** Нечисловые символы в поле «Код подразделения»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-012; ATOM-012; ASSERT-PASS-CUR-012; BSR 91
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для нечислового кода подразделения и сообщение `Код подразделения не в формате 000-000`.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Нечисловое значение кода подразделения: `123A56`.

### Шаги

1. Ввести `123A56` в поле `Код подразделения`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Нечисловое значение очищается после blur. Поле `Код подразделения` показывает сообщение `Код подразделения не в формате 000-000`.

### Постусловия

Не требуются.

## TC-PASSCUR-039

**№:** 39
**Название:** Форматирование кода подразделения
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-013; ATOM-013; ASSERT-PASS-CUR-013; BSR 92
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Допустимое значение: `123456`.

### Шаги

1. Ввести `123456` в поле «Код подразделения».
2. Проверить отображаемое значение поля «Код подразделения».

### Итоговый ожидаемый результат

Поле «Код подразделения» отображает значение `123-456`.

### Постусловия

Не требуются.

## TC-PASSCUR-040

**№:** 40
**Название:** Ввод более шести цифр в поле «Код подразделения»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-011; ATOM-011; ASSERT-PASS-CUR-011; BSR 91
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Недопустимое значение: `1234567`.

### Шаги

1. Ввести `1234567` в поле «Код подразделения».
2. Проверить отображаемое значение поля.
3. Очистить поле «Код подразделения».

### Итоговый ожидаемый результат

Седьмой числовой символ не вводится; поле содержит не более шести введенных цифр.

### Постусловия

Не требуются.

## TC-PASSCUR-041

**№:** 41
**Название:** Короткое значение в поле «Код подразделения»
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-011; ATOM-011; ASSERT-PASS-CUR-011; BSR 91; SO-CAL-7FE2E4D50CB184D5
**UI status:** confirmed
**Automation note:** UI evidence зафиксировал clear-on-blur для короткого кода подразделения `12345` и сообщение `Код подразделения не в формате 000-000`.
**Evidence:** `fts/AutoFin/PostFinal-v2-rc5/work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/evidence/per-tc/TC-PASSCUR-041.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Короткое значение кода подразделения: `12345`.

### Шаги

1. Ввести `12345` в поле `Код подразделения`.
2. Вывести фокус из поля.
3. Проверить значение поля и сообщение валидации.

### Итоговый ожидаемый результат

Значение `12345` очищается после blur. Поле `Код подразделения` показывает сообщение `Код подразделения не в формате 000-000`.

### Постусловия

Не требуются.

## TC-PASSCUR-042

**№:** 42
**Название:** Шестизначный код подразделения
**Тип:** позитивный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-011; ATOM-011; ASSERT-PASS-CUR-011; BSR 91
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Допустимое значение: `123456`.

### Шаги

1. Ввести `123456` в поле «Код подразделения».
2. Проверить, что поле отображает введенные шесть цифр в предусмотренном формате.
3. Очистить поле «Код подразделения».

### Итоговый ожидаемый результат

Поле «Код подразделения» отображает введенное значение как `123-456`.

### Постусловия

Не требуются.

## TC-PASSCUR-043

**№:** 43
**Название:** Поле «Код подразделения» обязательно для заполнения
**Тип:** негативный
**Приоритет:** высокий
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-031; ATOM-031; ASSERT-PASS-CUR-031
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

- Код подразделения: оставить пустым.

### Шаги

1. Оставить поле «Код подразделения» пустым.
2. Инициировать проверку обязательности поля.

### Итоговый ожидаемый результат

Поле подсвечено красным; под полем отображается текст «Обязательно к заполнению».

### Постусловия

Не требуются.

## TC-PASSCUR-044

**№:** 44
**Название:** Отображение поля «Код подразделения» в блоке «Паспортные данные»
**Тип:** позитивный
**Приоритет:** средний
**package_id:** WP-01-R2
**Трассировка:** OBL-PASS-CUR-010; ATOM-010; ASSERT-PASS-CUR-010; BSR 90
**UI status:** confirmed
**Automation note:** UI-путь и наблюдаемое поведение подтверждены UI-калибровкой; автоматизировать по шагам и ожидаемому результату TC.
**Evidence:** `work/ui-automation-prep/postfinal-v2-rc5-ui-calibration/ui-evidence-index.md`

### Предусловия

1. Открыть карточку `Заявка`.
2. Перейти к блоку `Паспортные данные`.

### Тестовые данные

Не требуются.

### Шаги

1. Проверить отображение поля «Код подразделения».

### Итоговый ожидаемый результат

Поле «Код подразделения» отображается.

### Постусловия

Не требуются.
