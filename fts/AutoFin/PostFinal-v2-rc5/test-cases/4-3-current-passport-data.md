...
1. Установить переключатель «Ввести вручную подразделение» в значение «Нет».
2. Ввести запрос `772-053` в поле «Код подразделения».
3. Проверить наличие точного предложения `ОВД ЗЮЗИНО Г. МОСКВЫ` в подсказках DaData.
4. Выбрать предложение `ОВД ЗЮЗИНО Г. МОСКВЫ`.

### Итоговый ожидаемый результат

Поле «Кем выдан» отображает значение `ОВД ЗЮЗИНО Г. МОСКВЫ`; порядок и количество предложений в списке не проверяются.
...
При вводе каждого тестового значения нечисловой символ не вводится в поле «Номер».
...
**Требуется подтверждение:** Какой точный UI-отклик подтверждает, что значение короче точной длины не принимается как валидное?
...
2. Инициировать проверку значения.
3. Зафиксировать фактический UI-отклик для значения `12345`.
4. Очистить поле «Номер».

### Итоговый ожидаемый результат

Фактический UI-отклик для значения `12345` зафиксирован; точное ожидаемое поведение требует UI-калибровки.
...
1. Установить переключатель «Ввести вручную подразделение» в значение «Нет».
2. Ввести запрос `772-053` в поле «Код подразделения».
3. Проверить, что список содержит точное предложение `ОВД ЗЮЗИНО Г. МОСКВЫ`.
4. Выбрать предложение `ОВД ЗЮЗИНО Г. МОСКВЫ` из списка.
5. Проверить значение поля «Кем выдан».

### Итоговый ожидаемый результат

Поле «Кем выдан» отображает выбранное значение `ОВД ЗЮЗИНО Г. МОСКВЫ`.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой точный наблюдаемый UI-отклик возникает при использовании значения «дата выдачи = дата 45-летия - 1 день; текущая дата = дата 45-летия + 90 дней» для элемента «Дата выдачи»?
...
2. Ввести рассчитанную дату выдачи в поле «Дата выдачи».
3. Инициировать проверку срока действия паспорта.
4. Зафиксировать фактический UI-отклик для подготовленного условия.

### Итоговый ожидаемый результат

Фактический UI-отклик для граничного условия зафиксирован; точное ожидаемое поведение требует UI-калибровки.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой точный наблюдаемый UI-отклик возникает при использовании значения «дата 14-летия» для элемента «Дата выдачи»?
...
2. Ввести рассчитанную дату в поле «Дата выдачи».
3. Инициировать проверку сохранения.
4. Зафиксировать фактический UI-отклик для условия `дата 14-летия`.

### Итоговый ожидаемый результат

Фактический UI-отклик для условия `дата 14-летия` зафиксирован; точное ожидаемое поведение требует UI-калибровки.
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой точный наблюдаемый UI-отклик возникает при использовании значения «дата выдачи = дата 20-летия; текущая дата = дата 20-летия + 90 дней» для элемента «Дата выдачи»?
...
**Статус oracle:** ui-calibration-required
**Статус тест-кейса:** candidate-ui-calibration
**Требуется подтверждение:** Какой точный наблюдаемый UI-отклик возникает при использовании значения «дата выдачи = дата 45-летия» для элемента «Дата выдачи»?
...
Фактический UI-отклик для условия `дата выдачи = дата 45-летия` зафиксирован; точное ожидаемое поведение требует UI-калибровки.
...
При вводе каждого тестового значения нечисловой символ не вводится в поле «Серия».
...
2. Инициировать проверку значения.
3. Зафиксировать фактический UI-отклик для значения `123`.
4. Очистить поле «Серия».

### Итоговый ожидаемый результат

Фактический UI-отклик для значения `123` зафиксирован; точное ожидаемое поведение требует UI-калибровки.
...
При вводе каждого тестового значения нечисловой символ не вводится в поле «Код подразделения».
