...
- `TC-CP-116443D9EB` и связанные ФИО-кейсы: DaData/autocomplete для полей `Фамилия`, `Имя`, `Отчество` контактного лица считать требованием/уточнением; invalid non-selected values очищаются после blur, обязательность проверяется через observed required state без выдуманного exact error text.
