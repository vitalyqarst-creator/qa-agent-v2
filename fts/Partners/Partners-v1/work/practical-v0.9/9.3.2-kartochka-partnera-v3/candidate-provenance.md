...
- В шести TC по кнопке «Сохранить» добавлены пропущенные коды Таблицы 7:
  `AS.36` для внутренней проверки дубликата и `AS.37` для сообщения и отказа
  в сохранении. У строки «Отменить или кнопка закрытия» код `AS.*` в ФТ не
  задан, поэтому её трассировка остаётся ссылкой на строку Таблицы 7.
...
Независимый re-review подтверждает immutable snapshot v3, зафиксированный в
`independent-review-v3-round-3-manifest.json`. После корректировки
трассировки актуальный текст ожидает fresh independent traceability re-review.
